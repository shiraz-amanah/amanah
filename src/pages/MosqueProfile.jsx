    ...
    <div className="min-h-screen bg-stone-50" style={{ fontFamily: "'Inter', sans-serif" }}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 md:p-8 max-w-5xl mx-auto flex items-end gap-4">
          ...
          <div className="text-white">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {mosque.verified && <span className="inline-flex items-center gap-1 text-[11px] bg-success-600 text-white px-2 py-0.5 rounded-full font-medium"><ShieldCheck size={11} /> Verified</span>}
            </div>
      ...
      <main className="max-w-5xl mx-auto px-5 md:px-6 py-6 md:py-8 space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setShowDonate(true)} className="bg-brand-900 hover:bg-brand-800 text-white text-sm font-medium px-5 py-2.5 rounded-xl inline-flex items-center gap-2 shadow-sm"><HandCoins size={16} /> Donate to this mosque</button>
          {!mosque.user_id && (
        ...
        {services.length > 0 && (
          <Section title="Services offered">
            <div className="flex flex-wrap gap-2">
              {services.map((s) => (
                <span key={s} className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 bg-brand-50 border border-brand-200 text-brand-800 rounded-lg">
                  <CheckCircle2 size={13} className="text-brand-600" /> {MOSQUE_SERVICES.find((x) => x.v === s)?.l || s}
                </span>
              ))}
            </div>
          </Section>
            ...
            <div className="flex flex-wrap gap-2">
              {facilities.map((f) => (
                <span key={f} className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 bg-stone-50 border border-stone-200 text-stone-700 rounded-lg">
                  <CheckCircle2 size={13} className="text-brand-600" /> {MOSQUE_FACILITIES.find((x) => x.v === f)?.l || f}
                </span>
              ))}
            </div>
          </Section>
          ...
          <Section title="Our teachers">
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {teachers.map((s) => {
                ...
                return (
                  <button key={s.id} onClick={() => onScholar?.(s)} className="text-left bg-stone-50 border border-stone-200 hover:border-brand-300 hover:shadow-sm transition-all rounded-xl p-3">
                    <div className="flex items-start gap-3">
                      <div className="w-11 h-11 rounded-full bg-gradient-to-br from-brand-400 to-brand-700 flex items-center justify-center text-white text-xs font-semibold flex-shrink-0 overflow-hidden">
                        {s.avatar_url ? <img src={s.avatar_url} alt="" className="w-full h-full object-cover" /> : scholarInitials(s)}
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-stone-900 truncate flex items-center gap-1.5"><GraduationCap size={13} className="text-brand-700 shrink-0" /> {s.name}</p>
                        <p className="text-xs text-stone-500 truncate">{[s.title, s.city].filter(Boolean).join(" · ") || "Scholar"}</p>
                        {s.dbs_verified && <span className="mt-1 inline-flex items-center gap-1 text-[10px] font-medium text-success-700 bg-success-50 border border-success-200 rounded-full px-1.5 py-0.5"><ShieldCheck size={10} /> DBS verified</span>}
                      </div>
                    </div>
                    {subs.length > 0 && (
            ...
            <div className="space-y-2">
              {events.map((e) => (
                <div key={e.id} className="flex items-center gap-3 border border-stone-100 rounded-xl p-3">
                  <div className="w-11 h-11 rounded-lg bg-brand-50 border border-brand-100 flex items-center justify-center flex-shrink-0 overflow-hidden">
                    {e.image_url ? <img src={e.image_url} alt="" className="w-full h-full object-cover" /> : <Calendar size={16} className="text-brand-700" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-stone-500">{fmtDate(e.date)}{e.time ? ` · ${e.time}` : ""} · <span className="text-brand-700">{typeLabel(e.type)}</span></p>
        ...
        {anns.length > 0 && (
          <Section title="Announcements">
            <div className="space-y-3">
              {anns.map((a) => (
                <div key={a.id} className="border-l-2 border-brand-300 pl-3">
        ...
        <Section title="Get in touch">
          <div className="flex flex-wrap items-center gap-2">
            {mosque.donation_url && (
              <a href={mosque.donation_url} target="_blank" rel="noopener noreferrer" className="bg-brand-900 hover:bg-brand-800 text-white text-sm font-medium px-4 py-2 rounded-lg inline-flex items-center gap-1.5"><HandCoins size={15} /> Donate</a>
            )}
            {mosque.website_url && (
