...
// banner + Ramadan times + the 30-day calendar. Premium brand/stone palette.
...
const MosquePrayerTimes = ({ mosque }) => {
  ...
  return (
    <section className="rounded-2xl overflow-hidden border border-brand-200 shadow-sm bg-white">
      ...
      {ramadan && (
        <div className="bg-gradient-to-r from-brand-700 to-brand-800 text-white px-5 py-3 flex items-center gap-2">
      ...
      <div className="p-5 md:p-6">
        <div className="flex items-center justify-between gap-2 mb-4 flex-wrap">
          <h2 className="text-lg font-semibold text-stone-900 inline-flex items-center gap-2" style={{ fontFamily: "'Fraunces', Georgia, serif" }}><Clock size={18} className="text-brand-700" /> Prayer times</h2>
        ...
        {nothing ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
              {PRAYERS.map((p) => (
                <div key={p.k} className="bg-brand-50/60 border border-brand-100 rounded-xl p-3 text-center">
                  <p className="text-sm font-semibold text-stone-900">{p.en}</p>
                  <p className="text-[13px] text-brand-800 mb-2" dir="rtl" lang="ar" style={{ fontFamily: "'Amiri', serif" }}>{p.ar}</p>
                  <div className="flex items-center justify-center gap-2 text-xs">
                    ...
                    <div><p className="text-[9px] uppercase tracking-wider text-brand-600">Iqamah</p><p className="font-mono font-semibold text-brand-800">{times[p.k]?.iqamah || "—"}</p></div>
                  </div>
                </div>
              ))}
            </div>
            ...
            {(jumuah.iqamah || jumuah.khutbah1 || mosque.jumuah_time || info?.sessions?.length) && (
              <div className="mt-4 rounded-xl bg-brand-900 text-white p-4">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <p className="text-sm font-semibold inline-flex items-center gap-2">Jumu'ah <span className="text-brand-200" dir="rtl" lang="ar" style={{ fontFamily: "'Amiri', serif" }}>{JUMUAH_AR}</span></p>
                  <div className="flex items-center gap-4 text-sm font-mono">
                    {jumuah.khutbah1 && <span><span className="text-brand-300 text-[10px] font-sans uppercase tracking-wider mr-1">Khutbah</span>{jumuah.khutbah1}{jumuah.khutbah2 ? ` / ${jumuah.khutbah2}` : ""}</span>}
                    <span><span className="text-brand-300 text-[10px] font-sans uppercase tracking-wider mr-1">Iqamah</span>{jumuah.iqamah || mosque.jumuah_time || "—"}</span>
                  </div>
                </div>
                {info?.sessions?.length > 0 && (
                  <ul className="mt-3 grid sm:grid-cols-2 gap-2">
                    {info.sessions.map((s, i) => (
                      <li key={i} className="text-xs bg-brand-800/60 rounded-lg px-3 py-2">
                        <span className="font-mono font-medium">{s.time || "—"}</span>{s.location ? ` · ${s.location}` : ""}{s.language ? ` · ${s.language}` : ""}{s.notes ? <span className="block text-brand-200/90 mt-0.5">{s.notes}</span> : null}
                      </li>
                    ))}
                  </ul>
                )}
                {info?.notes && <p className="text-xs text-brand-200/90 mt-2">{info.notes}</p>}
              </div>
            )}
