...
const CommunityCheckIn = ({ mosqueId, sessionId, authedUser, onHome }) => {
  ...
  if (notFound) return (
    <Shell>
      ...
      <button onClick={onHome} className="text-sm text-brand-800 hover:text-brand-900 font-medium">Go to Amanah</button>
    </Shell>
  );
  ...
  if (result) return (
    <Shell>
      ...
      {result.first_time && !result.already && (
        <p className="text-sm text-brand-800 mt-3 inline-flex items-center gap-1.5 bg-brand-50 border border-brand-200 rounded-full px-3 py-1"><PartyPopper size={14} /> Welcome — first time here!</p>
      )}
  ...
  return (
    <Shell>
      <p className="text-[11px] uppercase tracking-wider text-brand-700 font-semibold mb-1 inline-flex items-center gap-1"><MapPin size={12} /> {session.mosque_name}</p>
      ...
      {authedUser ? (
        ...
        <div className="space-y-3 text-left">
          ...
          <button
            ...
            disabled={submitting}
            className="w-full bg-brand-900 hover:bg-brand-800 disabled:bg-stone-300 text-white text-sm font-medium px-4 py-2.5 rounded-xl inline-flex items-center justify-center gap-1.5"
          >{submitting ? <Loader2 size={15} className="animate-spin" /> : <CheckCircle2 size={15} />} Check in</button>
          <p className="text-xs text-stone-400 text-center pt-1">Have an Amanah account? <button onClick={onHome} className="text-brand-800 hover:underline">Sign in</button> to check in as a member.</p>
        </div>
      )}
    </Shell>
  );
