  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-br from-brand-700 to-brand-900 text-white px-5 py-5">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="text-[11px] uppercase tracking-wider text-brand-200">Support your community</p>
          ...
          <p className="text-sm text-brand-50/90 mt-2">Every contribution sustains the masjid — its prayers, classes and care for the community. <span className="whitespace-nowrap">جزاك الله خيرا</span></p>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {PRESETS.map((p) => (
                <button key={p} onClick={() => { setAmount(p); setCustom(""); }} className={`py-2.5 rounded-xl border text-sm font-semibold ${custom === "" && amount === p ? "border-brand-500 bg-brand-50 text-brand-800" : "border-stone-300 text-stone-700 hover:border-stone-400"}`}>£{p}</button>
              ))}
            </div>
            <div className="relative mt-2">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400 text-sm">£</span>
              <input type="number" min="0" value={custom} onChange={(e) => setCustom(e.target.value)} placeholder="Other amount" className="w-full pl-7 pr-3 py-2.5 rounded-xl border border-stone-300 focus:border-brand-700 focus:ring-2 focus:ring-brand-100 outline-none text-sm" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {CAUSES.map((c) => (
                <button key={c} onClick={() => setCause(c)} className={`text-xs px-3 py-1.5 rounded-full border ${cause === c ? "border-brand-400 bg-brand-50 text-brand-800 font-medium" : "border-stone-300 text-stone-600 hover:border-stone-400"}`}>{c}</button>
              ))}
            </div>
          </div>
          ...
          <label className="flex items-start gap-2.5 bg-amber-50 border border-amber-200 rounded-xl p-3 cursor-pointer">
            <input type="checkbox" checked={giftAid} onChange={(e) => setGiftAid(e.target.checked)} className="mt-0.5 accent-brand-700" />
          ...
          {/* Pay */}
          <button disabled className="w-full bg-brand-900 text-white text-sm font-semibold py-3 rounded-xl inline-flex items-center justify-center gap-2 opacity-90 cursor-not-allowed">
