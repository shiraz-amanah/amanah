      ...
      <div className="bg-white border border-stone-200 rounded-2xl p-4 space-y-3">
        <div className="grid md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input value={city} onChange={(e) => setCity(e.target.value)} onKeyDown={(e) => e.key === "Enter" && run()} placeholder="City" className="w-full px-3 py-2 rounded-lg border border-stone-300 focus:border-emerald-700 focus:ring-2 focus:ring-emerald-100 outline-none text-sm" />
          </div>
        </div>
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <label className="flex items-center gap-2 text-sm text-stone-700"><input type="checkbox" checked={dbsOnly} onChange={(e) => setDbsOnly(e.target.checked)} className="rounded border-stone-300 text-emerald-700 focus:ring-emerald-200" /> DBS-checked only</label>
          <button onClick={run} disabled={loading} className="bg-emerald-900 hover:bg-emerald-800 disabled:bg-stone-300 text-white text-sm font-medium px-4 py-2 rounded-lg inline-flex items-center gap-1.5">{loading ? <Loader2 size={14} className="animate-spin" /> : <Search size={14} />} Search</button>
        </div>
              ...
              <div key={s.id} className="flex items-center gap-3 bg-white border border-stone-200 rounded-xl p-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-stone-900 truncate flex items-center gap-1.5">{s.name}
                    {s.dbs_verified && <span className="inline-flex items-center gap-0.5 text-[10px] text-emerald-700"><ShieldCheck size={10} /> DBS</span>}
                  </p>
