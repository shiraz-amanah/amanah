  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
        ...
        <span className="text-[11px] px-2 py-0.5 rounded-full border bg-emerald-50 border-emerald-200 text-emerald-700 whitespace-nowrap">{linked.size} linked</span>
      </div>

      <div className="relative mb-3">
        <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400" />
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search scholars by name, title or city…" className="w-full pl-9 pr-3 py-2 rounded-lg border border-stone-300 focus:border-emerald-700 focus:ring-2 focus:ring-emerald-100 outline-none text-sm" />
      </div>
            ...
            {sorted.map((s) => {
              ...
              return (
                <div key={s.id} className="flex items-center gap-3 bg-white border border-stone-200 rounded-xl p-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-400 to-emerald-700 flex items-center justify-center text-white text-xs font-semibold flex-shrink-0 overflow-hidden">
                  ...
                  <button onClick={() => toggle(s)} disabled={busyId === s.id} className={`text-[12px] px-3 py-1.5 rounded-lg border inline-flex items-center gap-1.5 ${isLinked ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-stone-300 text-stone-700 hover:border-emerald-300 hover:text-emerald-700"}`}>
