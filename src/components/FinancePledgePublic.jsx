...
const FinancePledgePublic = ({ sessionId, onHome }) => {
  ...
  return (
    <Shell>
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-2 my-4 text-center">
        <span className="text-sm text-emerald-900">Raised so far: <span className="font-semibold">{money(session.pledged_total)}</span>{session.target ? <> of {money(session.target)}</> : null} · {session.pledge_count} pledge{session.pledge_count === 1 ? "" : "s"}</span>
      </div>
      <div className="space-y-3 text-left">
        ...
        <label className="flex items-center gap-2 text-sm text-stone-700"><input type="checkbox" checked={giftAid} onChange={(e) => setGiftAid(e.target.checked)} className="rounded border-stone-300 text-emerald-700 focus:ring-emerald-200" /> I'm a UK taxpayer — add Gift Aid</label>
        {err && <p className="text-sm text-rose-700 flex items-center gap-1.5"><AlertCircle size={14} /> {err}</p>}
        <button onClick={submit} disabled={submitting} className="w-full bg-emerald-900 hover:bg-emerald-800 disabled:bg-stone-300 text-white text-sm font-medium px-4 py-2.5 rounded-xl inline-flex items-center justify-center gap-1.5">{submitting ? <Loader2 size={15} className="animate-spin" /> : <HandCoins size={15} />} Submit pledge</button>
      </div>
    </Shell>
  );
};
