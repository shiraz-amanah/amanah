    ...
    <div className="max-w-2xl">
      <div className="mb-6">
        <h2 className="text-2xl md:text-3xl font-semibold text-stone-900 tracking-tight mb-1 inline-flex items-center gap-2" style={{ fontFamily: "'Fraunces', Georgia, serif" }}>
          <CreditCard size={24} className="text-brand-700" /> Payments
        </h2>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-stone-200 rounded-2xl p-6">
          <div className="flex items-center gap-2.5 mb-3">
            <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-brand-800 bg-brand-50 border border-brand-200 px-2.5 py-1 rounded-full"><CheckCircle2 size={15} /> Connected</span>
          </div>
          ...
          <div className="flex flex-wrap gap-2 text-[12px]">
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border ${status.charges_enabled ? "border-success-200 text-success-700 bg-success-50" : "border-stone-200 text-stone-500"}`}><ShieldCheck size={13} /> Charges {status.charges_enabled ? "enabled" : "pending"}</span>
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border ${status.payouts_enabled ? "border-success-200 text-success-700 bg-success-50" : "border-stone-200 text-stone-500"}`}><ShieldCheck size={13} /> Payouts {status.payouts_enabled ? "enabled" : "pending"}</span>
          </div>
        </div>
      ) : connected ? (
        ...
        <div className="bg-white border border-amber-200 rounded-2xl p-6">
          ...
          <button onClick={connect} disabled={busy} className="bg-brand-900 hover:bg-brand-800 disabled:bg-stone-300 text-white text-sm font-medium px-5 py-2.5 rounded-lg inline-flex items-center gap-1.5">
        ...
        // ---- Not connected ----
        <div className="bg-white border border-stone-200 rounded-2xl p-6">
          <div className="w-11 h-11 rounded-xl bg-brand-50 border border-brand-100 flex items-center justify-center mb-3">
