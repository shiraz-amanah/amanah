  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={`shrink-0 inline-flex items-center gap-1 text-[10px] px-2.5 py-1 rounded-full font-medium uppercase tracking-wider ${open ? "bg-success-50 text-success-800 border border-success-200" : "bg-stone-100 text-stone-500 border border-stone-200"}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {open ? (
            <>
              ...
              {qrUrl && (
                <div className="text-center mt-3">
                  <a href={qrUrl} download={`checkin-${session.name.replace(/\s+/g, "-").toLowerCase()}.png`} className="text-xs text-brand-800 hover:text-brand-900 inline-flex items-center gap-1"><Download size={12} /> Download QR</a>
                </div>
              )}
            ...
            <>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={saveHeadcount} disabled={busy} className="bg-brand-900 hover:bg-brand-800 disabled:bg-stone-300 text-white text-sm font-medium px-4 py-2 rounded-lg inline-flex items-center gap-1.5 shrink-0">{busy ? <Loader2 size={14} className="animate-spin" /> : "Save"}</button>
              </div>
            </>
          )}
        </div>
        ...
        <div className={cardCls}>
          ...
          <div className="space-y-2 text-sm">
            <div className="flex items-center justify-between"><span className="text-stone-600 inline-flex items-center gap-1.5"><UserCheck size={14} className="text-brand-700" /> Named check-ins</span><span className="font-semibold text-stone-900 tabular-nums">{named}</span></div>
            ...
            <div className="flex items-center justify-between border-t border-stone-100 pt-2 mt-2"><span className="text-stone-900 font-medium inline-flex items-center gap-1.5"><CircleDot size={14} className="text-brand-700" /> Total footfall</span><span className="font-bold text-stone-900 tabular-nums">{totalFootfall}</span></div>
      ...
      {/* Live feed */}
      <div className={cardCls}>
        <p className="text-sm font-medium text-stone-900 mb-3 flex items-center gap-1.5">{open && <Radio size={13} className="text-success-600 animate-pulse" />} Check-in feed ({rows.length})</p>
            ...
            {rows.map((r) => {
              ...
              return (
                <div key={r.id} className="flex items-center gap-2.5 text-sm py-1">
                  <span className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-xs font-medium ${r.member_id ? "bg-brand-50 text-brand-800" : "bg-stone-100 text-stone-400"}`}>{r.member_id ? (r.member?.name || "M").slice(0, 1).toUpperCase() : <Users size={13} />}</span>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        ...
        {!showForm && <button onClick={() => setShowForm(true)} className="shrink-0 bg-brand-900 hover:bg-brand-800 text-white text-sm font-medium px-4 py-2 rounded-lg inline-flex items-center gap-1.5"><Plus size={14} /> Open session</button>}
      </div>
      {err && <p className="text-sm text-rose-700 flex items-center gap-1.5"><AlertCircle size={14} /> {err}</p>}
      ...
      {showForm && (
        <div className={cardCls}>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <button onClick={openSession} disabled={busy} className="bg-brand-900 hover:bg-brand-800 disabled:bg-stone-300 text-white text-sm font-medium px-4 py-2 rounded-lg inline-flex items-center gap-1.5">{busy ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />} Open session</button>
      ...
      {loading ? <div className="flex justify-center py-8 text-stone-400"><Loader2 size={20} className="animate-spin" /></div> : sessions.length > 0 ? (
        <div className="space-y-2">
          {sessions.map((s) => {
            const open = isOpen(s);
            return (
              <button key={s.id} onClick={() => setSelectedId(s.id)} className="w-full bg-white border border-stone-200 rounded-xl p-4 flex items-center gap-3 text-left hover:border-brand-300 group">
                <span className={`w-2 h-2 rounded-full shrink-0 ${open ? "bg-success-500" : "bg-stone-300"}`} />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-stone-900 group-hover:text-brand-800">{s.name}</p>
