...
> Last action (18 July 2026 — staff-card password-reset → localhost RECURRENCE fixed + WRONG-PROJECT dashboard incident): **A prod staff-card "Reset password" email dead-ended on `localhost:3000` AGAIN** — same Site-URL-fallback class as the morning recovery-link incident, **different call site**. **Root cause (code):** `StaffProfile.jsx` `doResetPassword` called `requestPasswordReset(row.email)` with **no `redirectTo`** → `resetPasswordForEmail(email, { redirectTo: undefined })` → GoTrue falls back to the project **Site URL**. The public `ForgotPassword` flow was unaffected — it passed an explicit allowlisted origin (`App.jsx:7283`, hardcoded prod). **Fix (create-account.js pattern — explicit root origin, never the Site-URL fallback; commit `927ec06`, Commit 1 alone):** `StaffProfile.jsx` + `ForgotPassword` now pass `window.location.origin`; **`auth.js requestPasswordReset` defaults `redirectTo` to `window.location.origin` when a caller omits it** so no future call site can regress. Swept all reset/recovery redirect call sites — `create-account.js` (root origin, morning fix), `MosqueStaffInviteAccept.jsx` (`window.location.href`), `App.jsx:7489` invite signup (hardcoded prod) — all clean. **No `VITE_*` bakes an app origin into the build** (only `VITE_SUPABASE_URL`, the DB endpoint) — ruled out as a vector. Build green. **WRONG-PROJECT / NON-PERSISTED DASHBOARD INCIDENT (deduced, then Shiraz-confirmed on the dashboard):** the morning Site-URL fix (Shiraz set prod Site URL → `youramanah.co.uk`) **could not have been effective on the prod project** — because if it had, this afternoon's undefined-`redirectTo` staff-card reset would have fallen back to `youramanah.co.uk`, not localhost. The recurrence PROVES the effective prod Site URL was **still `localhost:3000`** this afternoon → the morning dashboard change landed on the wrong project (or didn't save). Shiraz re-checked + corrected the **prod** Site URL now (+ dev where needed). **RITUAL REINFORCED (do NOT lose):** after ANY Supabase dashboard change, VERIFY you edited the intended project — `amanah` (prod, Vercel-served) vs `amanah-dev` (`pbejyukihhmybxxtheqq`) vs the `.env.local` `zgoyvztooyxqkcftwylr` project — a dashboard change that silently lands on the wrong project reads as done but isn't, and only resurfaces as a prod symptom (here: a second localhost dead-end that cost a full re-diagnosis). Pairs with the same-project-guard `ref === 'pbej...'` assert used for SQL. **PROD E2E PENDING (Shiraz, real inbox once live):** staff-card Reset password → link lands on `youramanah.co.uk/reset-password`, not localhost.
