...
>
> Session capstone (18 July 2026 — RBAC-E onboarding-hardening session): **Remote-staff onboarding now works genuinely end-to-end on prod (E2E PASSED), and the deferred parent marketplace signup is closed.** Four fixes shipped this session (all on `main`, all migrations live on prod); full detail in the dated entries below. **(1) Auto-provisioning (`26f7d6e`):** approval was an unwired dead path — it promoted the staff row but created no login account and left `profile_id`/`invite_status` unset, so the sign-in link was dead and login never resolved as staff. Now `OnboardingReview.approve()` → `provisionOnboardingAccount()` → owner-gated `create-account.js` provisions the auth user AND links the `mosque_staff` row (`profile_id` + `invite_status='active'`). **(2) Always-send set-password email + smart approval CTA (`bbb97e8`, `abd1828`):** the set/reset-password email now ALWAYS sends (it was skipped for pre-existing accounts — the first prod fail); the approval email's CTA routes by `last_sign_in_at` (never-signed-in → `/forgot-password`, returning → `/sign-in/staff`, fail-safe to set-password) so neither email can dead-end, and it does NOT mint a competing recovery token; admin sees a green/amber send-outcome note. **(3) Recovery-link root-origin hardening (`a302412`) + prod config:** the recovery link had resolved to `localhost:3000` — root cause was the prod Supabase **Site URL** left at localhost (GoTrue falls back to it when the `redirectTo` isn't allowlisted); Shiraz corrected Site URL + Redirect URLs, and `create-account` now passes the bare root origin so it resolves regardless of the allowlist. **(4) Parent-signup gate (`f2b577a`, migration 152):** the deferred "find a scholar" marketplace signup was reachable from the shared parent auth door and creating real accounts; `role='user'` self-signup now requires an enrolment signal (`has_pending_enrolment` — a mosque-enrolled child or a pending invite), login unchanged, `isInvite`/scholar/mosque unaffected, interest step + welcome email/alert de-marketplaced. **PROD E2E — PASSED (Shiraz, real fresh-email test):** full wizard → submit → approve → BOTH emails received → "Set your password to sign in" CTA worked → recovery link resolved to **`youramanah.co.uk/reset-password`** (not localhost) → password set → signed in → landed on the real staff portal **`/staff/portal`** with correct name/role/mosque. **Migrations: 149/150/151/152 all live on prod. Next migration 153.** **OPEN FOLLOW-UP:** the same-day stray-account scan surfaced two test rows — `shiraz+scholar-test100@…` (verified cleanup DO block provided; the delete needs the `mosque_staff` row [RESTRICT] AND the `profiles` row [`profiles.id`→`auth.users` is NO ACTION] removed before the auth user — GoTrue does NOT cascade `profiles`) and **`shiraz+8@savecobradford.co.uk` — DELIBERATELY LEFT UNTOUCHED**: a staff-type account, uncertain whether it's real/worth keeping, so do NOT delete without confirming first; a complete `pg_catalog` reference scan for it is in the parent-gate "Previous Last action" entry below.

> Last action (24 July 2026 — **WORKFORCE/TIMETABLE PHASE 1 hardening + calendar-term migration finalised on PROD**). Three queued items, all through. **(1) Migration 186 — `mosque_shifts` current-staff guard, APPLIED + HASH-VERIFIED ON PROD.** A `BEFORE INSERT OR UPDATE` trigger on `mosque_shifts` that rejects any `staff_id` resolving to a non-current `mosque_staff` row. Same defense-in-depth family as **157**: RLS on `mosque_shifts` (180) bounds *which mosque* an owner writes but places **no lifecycle bar on `staff_id`**, so the owner-only write path could schedule an offboarded or anonymised member — the exact gap `f53a729`'s commit note flagged and queued as 186. The reject predicate **mirrors `staffHelpers.isCurrentStaff` EXACTLY, not a restated rule** — `NOT current ⟺ anonymised_at IS NOT NULL OR offboarded_at IS NOT NULL OR status='offboarded' OR archived IS TRUE`; `status='suspended'` (product "Inactive") stays schedulable because `isCurrentStaff` includes it and the grid lists it. **SECURITY DEFINER, unlike 157's INVOKER** — this is an absolute integrity invariant with **no `current_user` exemption** (157 needed INVOKER only to branch `current_user` and exempt its definer RPCs), so the lookup must read true staff state regardless of caller RLS. errcode **23514**. DELETE is deliberately unguarded (a stale shift can always be removed); editing one for a now-former member is blocked by design. **Dev: `behcheck-186-dev.mjs` 8/8** — trigger present + `BEFORE INSERT OR UPDATE`; happy path inserts; offboarded / anonymised / archived / **UPDATE-path** all rejected with the **real 23514 error text** captured (not "it passed"); seed rolled back, dev clean. **Prod: pre-flight 0 violators; trigger attached + enabled; `md5(prosrc)` `3a8e88d83e8c5cb88975860d455c52fb` / len 909 — EXACT match to dev.** Prod pack emitted by **programmatic extraction** of the executable DDL from the migration file (PROD-PACK PRINCIPLE), so the hash comparison is meaningful. Committed `5a7e93c` (migration + behcheck). **(2) Migration 183 — strip `type='term'` from `mosques.academic_calendar`, APPLIED + VERIFIED ON PROD.** Final step of the term migration: terms now live solely in `academic_terms`; the parent calendar keeps holiday/exam/report_deadline events. **Gate satisfied:** the `a029d05` deploy removed `'term'` from the calendar editor and repointed term display to `academic_terms` (183's conditions a+b), and 182's insert was prod-verified first. The prod pack's **gate A** was hardened before running — A1 (count parity) **plus a new A2** that lists any calendar term with **no matching `academic_terms` row** (expect 0), stronger than counts alone — plus a pre/post non-term baseline proving no non-term event was dropped. Idempotent; Shiraz ran it directly. **(3) Date scan — CLEAN, no correction warranted.** The pre-fix grid's `iso()` used `toISOString()`, which in BST rolls a local-midnight day back one calendar day, so grid-written `shift_date` landed a day early (`f53a729` fixed it, shipped in `a029d05`). Read-only scan on prod: **`mosque_shifts` is completely empty (0 rows)** — no rows predate the fix, so **no BST correction needed** and no pre-existing 186 violators. **Ordering rule established (moot here but recorded):** any `shift_date` correction on prod must run **before** 186 attaches — a `shift_date + 1` UPDATE on a row referencing a now-non-current member would itself be rejected by the new guard (that is test 6). **Next migration: 187.** **Still open in this arc: the browser checks (class-clash EXCLUDE from 181, term CRUD), then Phase 2 — clock-in revival.**
>
> **Browser checks + parked madrasah items (24 July 2026).** **(1) Class-clash (181) — PASS both ways, proven end-to-end in the browser.** An overlapping same-teacher session was refused with a clear message ("Clash — this teacher already teaches Class 7 on Monday (17:00–18:00)"); a back-to-back session starting exactly at 18:00 saved fine (the `'[)'` bound is correct — abutting is not a clash). **(2) Term CRUD — BLOCKED, logged as a GAP.** There is **no UI to create an `academic_terms` row** — the Edit Class term dropdown shows only "No term", so `academic_terms` (180) can be linked but never populated from the app; term creation exists only via SQL. Needs a term-management surface (create/edit/delete terms per mosque). **(3) UX DEBT — two meanings of "term" one screen apart:** the Fee structure carries a **free-text TERM LABEL** while the class editor has the real **Term dropdown** (academic_terms) — same word, different concepts, adjacent on screen. Reconcile (rename one, or point the fee label at academic_terms). **DECISION: all madrasah-side work — the term-creation UI AND these two UX items — is PARKED until the Workforce tab is done. Do NOT start it.**
>
> **WORKFORCE TIMESHEETS — Phase 2 plan APPROVED (24 July 2026; build not started, brief surfaced for approval).** Three timesheet-ish tables exist: **058 `mosque_timesheets`** (weekly grid, dead), **085 `mosque_time_logs`** (clock-in/out log — real timestamps, break, *generated* `worked_hours`, pending→approved/rejected lifecycle, staff self-service RLS; **full data layer already in auth.js**, but orphaned — no UI), and **131 `mosque_staff_timesheets`** (manual owner-typed hours grid — what the current Timesheets sub-tab renders). **DECISION: consolidate on 085, retire the manual grid from the UI, admin-first Timesheets view** (list of clock logs for the week/range; admin manual add/edit/delete; approve/reject/reset via `setTimeLogStatus`; payroll CSV from real `worked_hours`; keep the audited salary reveal; `isCurrentStaff` filter on the add-log picker). **131 is FROZEN in place — UI stops pointing at it, no migration this phase; prod `mosque_staff_timesheets` count confirmed 0, nothing stranded.** **Guard decision (migration 187):** NO full 186-style guard on `mosque_time_logs` — **offboarded-staff logs stay ALLOWED (final-week back-pay is legitimate)** — but **reject INSERT/UPDATE where the staff row is ANONYMISED** (`anonymised_at is not null`): no fresh activity against a GDPR-erased record. Narrow sibling of 186/157. **SEQUENCING PIN: staff self-clock `/clock` page (NFC-tag URL target) is Phase 2b, starting IMMEDIATELY after this (2a) lands — a scheduled next step, NOT an unscheduled later increment.** Still deferred beyond 2b: log↔planned-shift variance, 058/131 table drops (freeze-don't-delete), overtime/rounding rules. **Migration 187 (mosque_time_logs anonymised guard) APPLIED + HASH-VERIFIED ON PROD (24 July 2026)** — pre-flight 0, trigger attached + enabled (BEFORE INSERT OR UPDATE), `md5(prosrc)` `5d753c48d9e87e50d81acf9078895b4e`/525, exact dev match (behcheck 7/7: current + offboarded allowed, anonymised INSERT+UPDATE rejected 23514). **Next migration: 188.** Commit 2 (WorkforceTab Timesheets → 085, retire the manual grid) is built on a BRANCH for a Vercel-preview clickthrough before merge.
>
> Previous Last action (21 July 2026 — **RETENTION-GATED ERASURE: migration 175 + Phase 1/Phase 2 lifecycle UI, PROD-LIVE, PROD-VERIFIED, DEPLOYED**). Closes the arc that began with the two Priority-1 defects. **THE FOUR DEFECTS FIXED TODAY (172–175):** **172** — GDPR erasure had raised 23514 on *every* call since 134 (`email = '[REDACTED]'` vs a CHECK admitting only a valid email or NULL); the abort happened on the FIRST statement after the ownership gate, so the employment table was never redacted and no audit row was written — total, not partial. **173** — the Permissions panel was dead for every staff member since 127 (`p.avatar_url` does not exist on `profiles`); the whole `profiles` join went, since it was a LEFT JOIN referenced only in the select list and both outputs (`profileName`, `profileAvatar`) were dead client-side. **174** — 172's fix then collided with 146's `(mosque_id, lower(email)) **NULLS NOT DISTINCT**` unique **index**, which made erasure a *once-per-mosque* action; narrowed to `WHERE email IS NOT NULL`. **The silent-failure swallow** — `confirmAnonymise` had no `else` branch and closed the modal unconditionally, so an irreversible, legally load-bearing action read as success when nothing had happened. **WHY NULL AND NOT A SENTINEL for erased emails — the decision to not re-litigate:** `mosque_staff.email` is a **join key**. `accept_staff_invite` (055) links an accepting account to a pre-existing row by `lower(email)` and **INSERTs a duplicate on a miss**, so a shared sentinel across many erased rows is a live false-link hazard — a future invite could match an erased record. `lower(null) = lower(x)` is NULL, never true, so a NULLed row is *permanently unmatchable*. NULL also passes 134's CHECK explicitly. A "syntactically valid sentinel" was considered and **rejected** for exactly this reason.
>
> **PHASE 1 — THE RETENTION DESIGN (migration 175).** `retention_eligible_at = greatest(end_date + 2 years, (first 5 April ON OR AFTER end_date) + 3 years)` — two years for right-to-work evidence, three years past the relevant tax year end for payroll/HMRC. **The HMRC leg almost always dominates**, which is why the old "retained 2 years" copy was wrong everywhere it appeared. The 5 April boundary is inclusive by design: an end date of exactly 2026-04-05 resolves to 2026-04-05 (the end of the tax year *containing* it), 2026-04-06 to 2027-04-05 — legally load-bearing, do not "fix" to strictly-after. **STORED, not derived**, so the date is auditable and stable if the rules later change; a generated column would silently rewrite history. The formula lives in ONE function (`staff_retention_eligible_at`) used by both `offboard_staff` and the backfill so they cannot drift, plus a client-side mirror in `staffHelpers` used **only** where no stored date exists yet (the offboarding checklist, which runs before the RPC has written one). **RULINGS:** **(a) `end_date` is REQUIRED on offboard with NO fallback** — the client blocks Continue and `offboard_staff` raises `end_date_required`. Previously it was a bare date input passed as `endDate || null`, so a blank date was reachable through the normal UI and would have produced a NULL retention date — i.e. a record that could **never** be erased, a silent one-way door. The formula still coalesces to `offboarding_completed_at` for LEGACY rows, and the guard's null branch survives as an unreachable safety net. **(b) `deleted_at` was SUPERSEDED, not reused.** It had been written as `now() + 2 years` but *read* as a tombstone (`get_mosque_staff_list` filters `deleted_at is null`) — nothing ever compared it to `now()`, so offboarded staff vanished from the Employees tab immediately and the "2 years" did nothing. Reusing a column whose two halves already disagreed would have carried that confusion forward; one retention concept is the point. **(c) The 157 privileged-column guard was EXTENDED** to `offboarded_at`/`anonymised_at`/`retention_eligible_at` — without it an owner could set the retention date into the past via the generic `updateMosqueStaff` `.update(updates)` passthrough and unlock their own erasure, defeating the entire gate. **NET EFFECT: offboarding is now a hard prerequisite of anonymising.** A never-offboarded record has no retention date, and the guard's null branch refuses it. `anonymise_staff` refuses **before its first write**, so a blocked attempt leaves the row completely untouched and logs nothing; on success it stamps `anonymised_at`, which is the flag list filters and the erasure register key on — **never sniff the `[REDACTED]` string**. `get_mosque_staff_list` gained the three columns (required for the UI to render the locked state at all), which forced a DROP+CREATE and a grant restore, tightened to `authenticated` only. **Verified by usage, not shape:** 18/18 at the RPC layer (offboard stores the right date; null end date refused; a retained row refuses with the row *and* audit log untouched; a direct client `.update()` of the retention date rejected 42501; time-travel then erase succeeds) + 10/10 in the browser.
>
> **PHASE 2 — LIFECYCLE UI (client-only).** Three tabs: **Employees** (current staff only — the header count follows the tab, so "N people" stopped counting people who had left or been erased), **Former staff** (offboarded-not-erased, still **named and searchable**, because statutory retention is precisely the period during which those records must remain intact and findable), and the **Erasure register** (skeletons: erased date + acting owner from the `staff_anonymised` audit row, and deliberately nothing else — re-deriving anything about the person would defeat the erasure). Partition helpers live in `staffHelpers` so the three views cannot disagree; **anonymised beats offboarded** so an erased record never surfaces as a named former employee, and the offboarded test tolerates legacy rows with no `offboarded_at`. **JUDGEMENT CALLS:** the **register is a staff tab, not under Compliance** — it is the terminal state of the same lifecycle (Employees → Former → Erased) and its source is `mosque_staff_audit_log`, not the DBS/RTW/Ofsted model Compliance is built on. The brief's "Employees excludes offboarded" and "revive the Offboarded filter" are in **direct conflict** (a filter for a category the tab excludes can never match); resolved as an **escape hatch** — Employees excludes offboarded by default, but explicitly selecting Status = "Offboarded" re-admits them. Org chart and compliance/Ofsted now scope to current staff (no count change; `isComplianceCountable` already excluded those rows). **Later polish:** the filter row is contextual per tab — Employees keeps the status pills, Former staff swaps to All / Retention active / Eligible to anonymise, the register has none. Rows with **no stored retention date** bucket under "Retention active" (they cannot be erased either) rather than earning a third chip.
>
> **TWO HOUSE RULES EARNED TODAY — both recorded in full in their own blocks below; do not re-derive them.** **(1) PROD-PACK PRINCIPLE** — prod apply packs are emitted by **programmatic extraction** from the migration file, never transcription. 175's pack was hand-retyped and silently dropped the in-body comments: it applied cleanly and behaved *correctly* (executable text byte-identical after normalisation) but `md5(prosrc)` diverged, leaving file, dev and prod holding three different sources. `md5(prosrc)` is the only instrument that catches this, because the functions worked. **(2) UI-HARNESS PRINCIPLE** — when a browser verification fails, check the selector against the actual rendered markup **before** concluding the product is broken. Two harness failures today (the Actions-menu selector; a `bodyRows()` first-line read that grabbed avatar initials) reported false product failures, and in both cases the correct answer was already visible in the probe's own output. **A wrong selector that FAILS is loud and self-correcting; one that PASSES is silent and ships a lie.**
>
> **OPEN PRODUCT DECISION — the bulk bar says "Suspend" while the canonical concept is "Deactivate"/"Inactive"** (stored `'suspended'`, shown as Inactive — see the staff status model block). The bulk action's own result message now reads "N people deactivated", so the button and its outcome disagree in the same breath. Leaning "Deactivate" to match canon, but it is user-facing product copy and **Shiraz's call** — deliberately not auto-resolved. **Also open, low priority:** `suspend_staff` is granted to **`PUBLIC` and `anon`**, unlike `get_mosque_staff_list` which 175 tightened to `authenticated` only. Not exploitable (SECURITY DEFINER with an ownership check, and `auth.uid()` is null for anon so the check fails), but inconsistent — tighten to authenticated-only whenever convenient. Found because a probe revoked EXECUTE from `authenticated` to force a failure and nothing changed: the PUBLIC grant kept it working.
>
> **`scrollbar-hide` IS A DEAD CLASS — 16 live usages across 10 files (5 in App.jsx), defined NOWHERE (no Tailwind plugin, `plugins: []`, no CSS, no package) and confirmed absent from the BUILT css. It is inert today, so those 16 surfaces are showing whatever scrollbar the platform draws. Defining it would not be a no-op — it would silently change all 16 at once, so treat that as its own scoped change with its own verification, never as a drive-by fix.** (The Staff tab row deliberately does not use it; see the 21 July mobile-tabs fix.)
>
> **BOARD CLEARED (21 July 2026).** The three items left open at the end of the retention arc are all closed. **(1) Prod former-staff data** — closed by Shiraz. **(2) The `deleted_at` filter — closed by MIGRATION 176** (below). **(3) Next migration** — now **177**.
>
> **MIGRATION 176 — `get_mosque_staff_list` drops the dead `deleted_at` predicate (dev + prod, both verified by `md5(prosrc)`).** The last live reference to the superseded concept. It had been **always-true since 175** (which stopped the write and cleared every value), so this is **trap removal, not a bug fix** — the clause sat in the one function the whole staff lifecycle reads through, where a future reader has every reason to assume a filter in that position is load-bearing: they might preserve it through a rewrite, or "restore" writes to `deleted_at` to make it meaningful again, quietly reintroducing the two-concepts problem 175 removed. **No DROP needed, unlike 173/175** — the `RETURNS TABLE` shape is unchanged, so `CREATE OR REPLACE` is legal, and because nothing is dropped **the grants survive untouched** (measured before and after: `authenticated, postgres, service_role`, anon absent — no revoke/grant block belongs in that file). Built per the PROD-PACK PRINCIPLE: the statement was **extracted programmatically** from 175 and one line deleted, with build-time assertions that exactly one occurrence existed and zero remain. Dev: `7de6308a…`/1155 → **`64ebd0e8ee8aec1749e8a833ffd14681`/1122** (−33 bytes = exactly the clause), `position('deleted_at' in prosrc)` 1098 → **0**, `mosque_staff` row count unchanged at 13. **The `mosque_staff.deleted_at` COLUMN is deliberately left in place** — unreferenced by any writer or reader after this, and dropping a column is a separate, heavier decision. **Prod applied + verified 21 July 2026**, all three checks matching dev exactly: pre-flight `rows_with_deleted_at` **0** (the real gate — a non-zero there would have meant the predicate was load-bearing on prod, and the apply would have had to stop), `64ebd0e8ee8aec1749e8a833ffd14681`/1122, grants `authenticated, postgres, service_role` with **anon absent** (confirming grants survive a REPLACE on prod too, not just dev).
>
> **The false-alarm detour, recorded because it cost two turns and nearly corrupted this entry.** A probe reported dev as `7de6308a…`/1155 — the *pre-176* value — and was read as "176 was never applied to dev; only the file was built and self-checked." It was applied. `7de6308a…`/1155 is what **prod** returned, since prod was still on 175 at that point; the probe had almost certainly run against the wrong project's SQL editor. Two things follow. **(1) A hash with no project label is not evidence** — dev and prod diverge by design between apply and promote, so that is exactly the window where an unlabelled hash is most likely to be misread, and it reads as a *regression* rather than as a *different database*. Record the ref alongside the hash. **(2) It nearly produced a "correction commit" rewriting this entry to say 176 had not been dev-applied** — i.e. replacing a true record with a false one, on the strength of a misattributed number. What stopped it was re-reading what the entry actually claimed: a before/after grants comparison and `position('deleted_at' in prosrc)` 1098 → 0 are *post-apply measurements against a live database*, and cannot be produced by building a file and self-checking it. **A journal entry containing measurements that could only come from a live database is itself evidence about that database's state** — check the claim's internal consistency before believing a number that contradicts it. Also corrected en route: "no SQL exec path to dev" — `scripts/behcheck-*.mjs` connect directly via `DEV_DATABASE_URL` with `pg`, so dev is fully probeable from here (`scripts/probe-176-dev.mjs` is that probe, in the same idiom).
>
> **SEEDED BEHCHECK for Former staff + Erasure register (`scripts/behcheck-176-lifecycle-dev.mjs`, 31/31 on dev, seed rolled back).** The retention arc shipped two lifecycle tabs that had **never been exercised with data**: dev and prod both held zero offboarded and zero anonymised rows, so every "counts unchanged" pass on those tabs — including the one recorded in this entry — was an **empty-set pass**, which is not evidence. The probe seeds through the **real RPCs** and asserts the lifecycle end to end: `end_date_required` on a null end date; offboard stamps `offboarded_at` + `retention_eligible_at` and lands in Former; erasure inside retention raises `retention_active` leaving the row **completely untouched with zero audit rows**; a 2019 leaver reaching **2022-04-05** via the *5-April+3y* arm of the formula (not the `end_date`+2y arm — the less obvious branch, now covered) erases with `email` **NULL** and moves Former → Erasure; **no row acquires `deleted_at` anywhere in the lifecycle** (the assertion that would have caught 176 had it been unsafe); and the three buckets are a **true partition** (sum to total, every row in exactly one). Each expected failure runs in its own **SAVEPOINT** — which is also what gives the "untouched" assertion its force: the savepoint rollback undoes *nothing*, because a correct `anonymise_staff` refuses before its first write. Verifies the **data layer and partition functions, not React rendering**.
>
> **The `isFormer` finding — a restated predicate that agreed by coincidence. THIRD occurrence of this class today.** Ad-hoc verification used `offboarded_at is not null` as "former". The **shipped** predicate is broader: `!isAnonymised && (offboardedAt || status === 'offboarded' || archived)` — it deliberately tolerates **legacy leavers that predate 175** and so carry no `offboardedAt`. The two are not equivalent, and the ad-hoc one **under-counts**. It passed only because **dev holds 0 `offboarded_at`, 0 `archived`, and 0 `status='offboarded'` rows** — so the two predicates agreed on the **EMPTY SET**. (Note the earlier framing "one real leaver satisfied both" was wrong: there was no leaver at all on dev. Agreement on an empty set is the *weakest possible* agreement, and worth distinguishing — it means the disagreeing branch was never reached, not that it was tested and matched.) **Open question for prod — CLOSED.** Prod does have former/erased rows (Zain, Fatima), so the two predicates may already diverge there; the ad-hoc form should not be reused against prod without re-deriving it. Checked on prod 21 July 2026: `adhoc 1 = shipped 1` — they agree there too, on one real row (Zain) rather than the empty set. Still coincidence, not equivalence: Zain carries `offboarded_at`, so the `archived`/`status` branch remains unreached on both databases. **The rule: verification code must EXECUTE the shipped predicate, never restate it.** A restated predicate can agree by coincidence on today's data and diverge silently later — and the divergence appears as a quiet under-count, not an error. `behcheck-176` therefore **extracts `isAnonymised` / `isFormer` / `isCurrentStaff` / `shapeStaffListRow` from `src/lib/staffHelpers.js` and evaluates them** rather than reimplementing, so it cannot drift from the UI. (Extraction also sidesteps `staffHelpers` importing `supabaseClient`, whose `import.meta.env` will not load under plain node.) **This is now the third instance of the same class in one day** — the Actions-menu selector, the `bodyRows()` first line, and this — so treat it as a **pattern, not a one-off**: whenever verification names a thing the app also names (a selector, a row set, a predicate), the check must reach for the app's own definition. Restating it converts a test into a second, unmaintained implementation that is asserted against the first only by luck.
>
> **HEADER-COUNT DEFECT, found by Shiraz after Phase 2 shipped — and a lesson about what the verification asserted.** The Staff page header read **"3 people" above an "Employees (1)" tab**: it used the raw `staff` total while only the tab *label* was scoped. **Phase 2's probe asserted the tab label and never compared it to the header**, so a disagreement sitting inches away on the same screen passed clean. Worse, the comment above the partition **already claimed** "the header count follows the tab so 'N people' is honest" — the comment was aspirational and the test agreed with the comment instead of the screen. Fixed by sourcing the header from the same value as the active tab's label, so the two cannot diverge; deliberately **unfiltered**, so typing in search narrows the table without moving the header. Org Structure shares the Employees count (it draws the current organisation); Onboarding keeps the current headcount because its rows are review sessions, not staff records. **Re-verified with DELIBERATELY DISTINCT per-tab counts (2 current / 3 former / 1 erased) so a stale value could not coincidentally match** — 11/11, including in-app tab clicks rather than reloads, singular/plural wording, and the search-must-not-move-it case. **Generalise: when two numbers describing the same thing appear on one screen, assert them AGAINST EACH OTHER, not each against your own expectation.**
>
> Previous Last action (21 July 2026 — **BOTH PRIORITY-1 DEFECTS CLOSED + a third found by usage verification: migrations 172/173/174, ALL PROD-APPLIED, PROD-VERIFIED, DEPLOYED**): The two pre-existing Priority-1 defects listed below are **FIXED**, and a third surfaced during verification. Pushed `3139d21..1de2bf8`, prod deploy READY in 53s. **172 — GDPR erasure repaired.** `anonymise_staff` had raised 23514 on **every** call since migration 134 (129 wrote `email = '[REDACTED]'`; 134 later added a CHECK admitting only a valid email or NULL, and did not touch 129's function). Blast radius was **total, not partial**: the `mosque_staff` UPDATE is the first statement after the ownership gate, so it aborted **before** the employment-table redaction **and before** the `staff_anonymised` audit insert — nothing was redacted and no trace of the attempt was written. Fixed to `email = null`. **NULL was chosen over a sentinel deliberately:** `mosque_staff.email` is a **join key** — `accept_staff_invite` (055) links by `lower(email)` and INSERTs a duplicate on a miss, so a shared sentinel across erased rows is a live false-link hazard; `lower(null) = lower(x)` is NULL, never true, so a NULLed row is permanently unmatchable. **173 — Permissions panel repaired.** `get_mosque_employees` selected `p.avatar_url` from `profiles`, which does not exist. The whole `profiles` join was dropped rather than the one column: it was a LEFT JOIN referenced only in the select list (`p.name`, `p.avatar_url`), so it filtered nothing, and **both** outputs were dead client-side (`profileName` and `profileAvatar` were each their only occurrence repo-wide). Signature change → DROP + CREATE; the `revoke ... from public, anon` is **load-bearing**, because `CREATE FUNCTION` grants EXECUTE to PUBLIC by default and Supabase's default privileges add anon, so a bare grant-to-authenticated would have left anon executable. **174 — THE LESSON. A unique INDEX is invisible to `pg_constraint`, and shape verification would have shipped a broken feature.** 172 passed every shape check, then the real-usage probe hit `duplicate key ... mosque_staff_mosque_email_unique`: migration 146 had created `(mosque_id, lower(email)) **NULLS NOT DISTINCT**` — expressed as a unique **index** (it had to be, for the `lower()` expression), so it **never appeared in `pg_constraint`** and the pre-write sweep missed it. Effect: only the **first** staff member at a mosque could ever be erased; at a mosque already holding one NULL-email row, even the first erasure failed. GDPR erasure cannot be a once-per-mosque action. 174 narrows it to `WHERE email IS NOT NULL` — real-email uniqueness (the thing 055 actually needs) is preserved untouched, and duplicate real emails still raise 23505 so `AddStaffModal`'s error translation is unaffected. **Verified by USAGE, not shape, throughout:** 173 — the employees panel *rendered rows* in a real browser (a 200 from the RPC was explicitly treated as insufficient); 172 — the erasure was driven through the actual Actions → Anonymise → confirm UI, and the **failure** path was proven by temporarily forcing the pre-172 body back in, confirming the error surfaces and the modal stays open, then reverting and re-verifying by md5 **and** functionally; 174 — 9/9 including two erasures at one mosque, an erasure alongside a pre-existing NULL-email row, and a case-insensitive duplicate-real-email collision. **A harness bug nearly produced a false negative:** the first browser run reported 5 failures that were purely my selector failing to find the Anonymise affordance (it sits behind the Actions menu) — vacuous assertions, not product defects. **Also shipped:** `StaffProfile.confirmAnonymise` had **no else branch** and closed the modal unconditionally, so the 23514 dismissed the dialog silently with no banner and no console trace — an irreversible, legally load-bearing action reading as success when nothing had happened. Now logs, shows an amber banner, and keeps the modal open. **Prod hashes verified:** `anonymise_staff` 1250/`917901c8e5772fef0622ae32cbab69ea`, `get_mosque_employees` 582/`38097296857b65c8c1890ee51ef78973`, index `WHERE (email IS NOT NULL)`, anon absent from grants. **Next migration: 175.**
>
> Previous Last action (21 July 2026 — **HR DEFECT CLOSURE: migrations 169/170/171 + four client commits, ALL DEV-PROVEN, NONE PROD-VERIFIED, NOT MERGED**): Five HR defects closed on branch `hr-defect-closure` (off `main` @ `19ad074`). **Client:** `d876848` remote-invite now carries pay (`salary_pence`/`hourly_rate_pence`/`hours_per_week`) in the `employment` blob — it was captured on step 3 and then silently dropped at the invite call, reaching only the contract HTML; mirrors the in-house **zero-hours guard** so a type switch can't put an annual salary on a casual-worker record. `cdb49d2` contract signer labels now read a per-type **`party`** field on `TYPES` (Employee/Worker/Volunteer/Contractor) — clause 1 already had the right four-way answer as a hardcoded ternary while the signature block had a *second* copy testing `m.employee === false`, a key **no type ever sets to false** (the three that needed it simply omit it), so every contract signed "Employee" and the `"Contractor/Volunteer"` branch was unreachable; clause-1 output verified byte-identical for all six types. `d28f47c` nationality capitalised **on display only** (render site, not the post-save merge; stored value provably unchanged). `0b8ecb3` `next_of_kin` removed from the Personal panel — one free-text column whose own placeholder asked the owner to hand-concatenate the exact name+phone pair the structured, wizard-**required** `emergency_contact_name`/`_phone` already store and render one line above. **Migrations (dev only): 169** `approve_onboarding_session` persists pay — it wrote `contract_type`/`hours_per_week`/`salary_rate` and never `salary_pence`/`hourly_rate_pence`, read `emp->>'contract_type'` while the invite writes `employment_type`, and its bare `excluded.*` on-conflict **BLANKED admin-set values on re-approve**; now pence columns + key fallback + `coalesce(excluded, existing)` on every line except `student_loan` (where `false` is real). **170** `update_employee_permissions` audits permissions-only edits — `v_old_permissions` was never even SELECTed, so the most privilege-sensitive write in the staff model wrote **zero** audit rows. **171** drops `next_of_kin` (functions recreated first, then the column). **EVIDENCE:** 169 → behcheck 20/20 (incl. a non-destruction case proving pre-set salary/hours/address/emergency-contact survive an approve that omits them) + a **14/14 full-wizard UI run** on preview (invite £28,500/37.5h → 9 wizard steps incl. RTW upload → approve → `salary_pence=2850000` → panel shows it behind the 163 reveal gate). 170 → behcheck 20/20 (permissions-only audits; no-op writes nothing; preset-only and classes-only still audit; **`'custom'` guard intact**; update-only, never insert). 171 → `behcheck-168-dev` 11/11 + 13-pass Personal-panel run. **⚠️ ALL PROD GATES REMAIN OPEN.** The prod `pg_get_functiondef` output was requested six times and arrived as an **unfilled `[paste …]` placeholder every time**, so nothing is verified on prod and **the branch was deliberately NOT merged** — merging publishes to `main`, which auto-deploys prod. Standing rule from the brief: no merge until 169, 170 AND 171 are applied and probed there. **A better instrument was left ready:** `select p.proname, length(p.prosrc), md5(p.prosrc) …` returns four short rows instead of a 200-line body; expected values (file and dev hash identically) — `approve_onboarding_session` 8097/`5e7382c905b508f173001b6d87c25e41`, `update_employee_permissions` 2714/`8fa90ae77d0fb9be336a8bc8aad6d122`, `get_staff_sensitive` 983/`cd442b7e3bdf045352d5993951942a46`, `anonymise_staff` 1258/`770cf7fc8b0d8b5a437503a9e80da2e7`. **`md5(prosrc)` hashes only the body, so it is immune to server-side header formatting — and it catches the exact failure "it's applied" cannot: a semicolon-split partial paste leaves a function that still exists and still answers to the right name with the wrong body.** **Next migration: 172.**
>
> **✅ BOTH CLOSED 21 July 2026 — kept for provenance.** (a) fixed by **migration 173**, (b) by **migration 172**; both prod-applied, prod-verified and deployed — see the Last action entry above. The diagnosis below is accurate as written; only the "not fixed here / needs migration 172" framing is now historical. **One correction to (b):** the suggested alternative "redact to a syntactically valid sentinel" was considered and **rejected** — `mosque_staff.email` is the join key `accept_staff_invite` (055) matches on, so a shared sentinel across erased rows is a false-link hazard. NULL is the only value that cannot false-match. (And note 174: NULL then collided with 146's `NULLS NOT DISTINCT` unique **index**, which `pg_constraint` does not show.)
>
> **⚠️ PRIORITY 1, PRE-FIRST-DEMO — TWO PRE-EXISTING DEFECTS FOUND BY THE 170/171 UI EXERCISES (neither caused by this session, neither fixed here — both need migration 172).**
> **(a) The Permissions panel is DEAD for every staff member, since migration 127.** `get_mosque_employees` (`127_rbac_rpcs.sql:226`) selects **`p.avatar_url` from `profiles`** — that column does not exist (`profiles` has `avatar_initials`/`avatar_gradient`; `avatar_url` is a **`scholars`** column, migration 040). plpgsql resolves column refs at execution time, so the function *created* fine and fails on **every call**: `column p.avatar_url does not exist` → 400 → `getMosqueEmployees` swallows it and returns `[]` → `PermissionsSection` sets `emp = null` → the panel renders **"This staff member doesn't have dashboard access"** for everyone. An owner cannot view or edit RBAC from the staff profile. Fix = drop or swap that one column. **Why nothing caught it:** the only other permissions writer, `applyRoleDefaults` (`staffHelpers.js:230`), queries `mosque_employees` **directly**, not via the RPC — so role assignment works and session 167's probe passed while the panel was dead. `StaffProfile.jsx:1215` is the sole caller. **170's UI exercise is blocked on this and must be re-run when 172 ships** (`scripts/clickthrough-170-permissions.mjs` is committed and ready); 170's behaviour is currently proven at the RPC level only.
> **(b) `anonymise_staff` CANNOT RUN AT ALL — GDPR right-to-erasure is dead, since migration 134.** It sets `mosque_staff.email = '[REDACTED]'`, but 134 added `CHECK mosque_staff_email_format` (**valid email OR NULL**). `'[REDACTED]'` is neither, so the UPDATE always raises `violates check constraint "mosque_staff_email_format"` and **`mosque_staff_employment` is left entirely un-redacted**. Proven pre-existing: the identical line is in `129:304`, in the captured pre-171 live body, and in 171 — 171 only removed `next_of_kin` from the list. Fix = set `email = null` (the CHECK explicitly allows NULL) or redact to a syntactically valid sentinel. **Given the ICO registration this arguably outranks (a).**
>
> **UI-HARNESS PRINCIPLE (21 July 2026): when a browser verification fails, check the SELECTOR against the actual rendered markup before concluding the feature is broken.** Two harness failures in one day shared a single root cause — reading the wrong DOM slice and reporting it as a product defect. **(1)** The migration-172 erasure run reported **5 failures**; the selector looked for a bare `Anonymise…` button, but that affordance sits behind the **Actions menu** (`actionsMenu` only mounts when `actionsOpen`), so the dialog never opened and every downstream assertion was **vacuous**. **(2)** The Phase-2 filter run reported **2 failures**; `bodyRows()` took `innerText.split('\n')[0]`, which on those rows is the **avatar-initials** line (`"LP"`, `"EP"`) rather than the name — the filter had in fact returned exactly the right single row each time, visible in the very output that was being called a failure. **Standing check before believing a UI failure:** dump the relevant `innerText`/button list and confirm the selector matches what is actually on the page; confirm the route/section really renders the target (172's first run also pointed at `?section=account` when the danger zone lives on the **Overview** branch); and re-read the probe's own printed data — in both cases the correct answer was already in the log. **Note the asymmetry: a wrong selector that FAILS is loud and self-correcting; a wrong selector that PASSES is silent and ships a lie.** That is the real reason to fix the harness and re-run to green rather than explaining failures away in prose.
>
> **PROD-PACK PRINCIPLE (learned the hard way, 21 July 2026): prod apply packs are emitted by PROGRAMMATIC EXTRACTION from the migration file — never transcription.** Migration 175's pack was hand-retyped into the reply instead of being cut from `migrations/175_retention_gated_erasure.sql`, and the in-body `--` comments were dropped in the process. The result applied cleanly and behaved correctly — after stripping comments and collapsing whitespace the executable text was **byte-identical** — but `md5(prosrc)` diverged (`anonymise_staff` prod `b6e3563c…`/1445 vs dev `dbaa73bf…`/1727; `offboard_staff` prod `3d0bf970…`/1087 vs dev `ebfb4ebc…`/1302), so file, dev and prod held three different sources for the same function. **`md5(prosrc)` is what caught it** — nothing else would have, because the functions worked. **The fix pattern:** `file.indexOf(startMarker)` → `file.indexOf(endMarker)` → slice; then hash the extracted body and assert it equals the expected md5/len **before** emitting anything. If the extraction does not match, abort rather than emit. Diagnosis was confirmed by reconstructing the emitted text and hashing it to prod's exact value, which proves the divergence is the transcription and not editor mangling or a partial paste. **Corollary: byte-identical provenance across file → dev → prod is only guaranteed when the same bytes travel all three places.** A second transit corruption (three duplicated lines inside `anonymise_staff`) then hit the hand-carried re-apply block in chat, which is why the canonical bodies now go to a FILE and the file is what gets read.
>
> **BEHCHECK PRINCIPLE (learned the hard way, 21 July 2026): a behcheck must PROBE the live object, never RE-APPLY its own migration.** `behcheck-168-dev.mjs` re-applied migration 168 on every run. Once 171 superseded 168's `get_staff_sensitive`, running the 168 check **silently reverted 171** and left a live function selecting a dropped column — every call then failed with `column e.next_of_kin does not exist`. Dev was repaired by re-applying 171's two function halves *from the file*, and the self-apply was removed. **Any behcheck that installs its own migration is a landmine the moment a later migration supersedes it.** Audit the older `behcheck-*.mjs` scripts for the same pattern before trusting them.
>
> **CORRECTIONS TO STANDING NOTES (21 July 2026):** **(1) No prod credentials exist locally, BY DESIGN.** The old claim that `.env.local` holds the prod service pair is **wrong** — `.env.local` no longer carries any service key; `.env` has `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` pointing at **DEV `pbej`**, and `.env`'s `VITE_SUPABASE_URL` points at **PROD `zgoy`** (harmless: `.env.local` wins for Vite, so `npm run dev` resolves to dev — verified via `loadEnv`). The prod service key **401s**. All prod SQL therefore runs through Shiraz in the Supabase editor; CC applies and probes **dev only**. **(2) Migration 151's CHECK lives on `mosque_staff.employment_type`**, not on `mosque_staff_employment` — values `employed_full_time, employed_part_time, self_employed, volunteer, contractor, zero_hours` (verified on dev). **`contract_type` has NO CHECK** — a future candidate. **(3) The 12/12 function list in CLAUDE.md was STALE.** Still 12/12, but the real `api/` is: `admin-brief, ai-match, create-account, daily, embed, health, moderate-message, onboarding-upload, score-profile, search, send-transactional, stripe-connect`. `create-daily-room`+`get-meeting-token` were consolidated into `daily.js`; `send-staff-invite` is gone; `create-account` and `onboarding-upload` are new. **(4) `.gitignore` now covers `.env*`** — `vercel env pull --cwd <repo>` wrote a real decrypted `.env.preview` into the repo root, and `.env`/`.env.local` alone did not cover it. **(5) Preview deployments target DEV**, proven empirically rather than assumed: the dashboard marks `VITE_SUPABASE_URL` Sensitive/unreadable in **both** Preview and Production, so the deployed **bundle** was the ground truth — `/assets/index-BByrc80y.js` contains exactly one Supabase host, `pbejyukihhmybxxtheqq`, and zero occurrences of `zgoy…`. The Preview-scoped `SUPABASE_SERVICE_ROLE_KEY` is confirmed **valid**, not merely present: the RTW upload succeeded through `/api/onboarding-upload`. **(6) `salary_rate` is no longer written by the approval path** (it was always null from that path); the column itself is untouched.
>
> **FOLLOW-UPS LOGGED, NOT DONE:** **(i)** `approve_onboarding_session` does **not** set `mosque_staff.employment_type`, and the client contract `TYPES` vocabulary (`full_time`/`part_time`/…) **≠** the CHECK vocabulary (`employed_full_time`/…) — needs a mapping decision in its own session. **(ii)** 169's coalesce upsert means **approval can add a field but never blank one** — deliberate, and the reason re-approval no longer destroys admin edits. **(iii) Prod junk:** `mosque_employees` duplicates — mosque `290655fd…` has a null-profile pair plus one profile with **3 rows**; eventual fix = cleanup + unique constraint + a decision on the null group (**a plain UNIQUE will not catch it**, since NULLs are distinct). Parked with the RBAC-D junk rows. **(iv)** `notice_period_days` backfill still parked — note `AddStaffModal` writes `notice_period_days` while `update_staff_employment` (162) uses `notice_period_employer_weeks`/`_employee_weeks`.
>
> Previous Last action (20 July 2026 — **PRESET FALLBACK DEFECT FIXED + PROVEN: `41420c8` live on prod**): The last known live defect in the role→permissions chain is closed. `applyRoleDefaults` (`src/lib/staffHelpers.js`) now **expands the preset via `getDefaultPermissions(rolePreset)`** on the fallback path and sends the full 13-key blob, replacing the old `p_permissions: null` that the RPC's `coalesce(p_permissions, permissions)` turned into a no-op — the label moved, access never did. **Two lines** (extend the import; guard-and-expand at the RPC call), no migration, no new RPC, no new columns. Deploy `dpl_C8osqRZjJdhpGdR8DhHATuTjebkX` READY on `youramanah.co.uk`, 12/12 functions. **PROVEN BY THE REAL-UI PROBE, NOT JUST THE AUDIT:** re-running `scripts/clickthrough-167-priority.mjs` on dev now returns **5/5 (was 4/5)** — **CASE 2 flipped ❌ → ✅**, `permissions` genuinely rewritten to the full teacher expansion (`classes:'own' … messages:'own'`) where before it was byte-identical across the change. CASE 1 unchanged: the granular blob still beats the preset. **⚠️ `'custom'` IS GUARDED (option (i)) — this is the one judgement call, and it was made because the A/B/C question went unanswered at commit time.** `getDefaultPermissions` returns the all-false `EMPTY_PERMISSIONS` for any key not in `PRESET_ROLES`, and `'custom'` is exactly such a key (valid in the RPC CHECK, selectable in the roles UI), so blind expansion would have **STRIPPED a live person's dashboard access to nothing** on a role change. `'custom'` therefore keeps label-only behaviour. **✅ RESOLVED — option (i) explicitly confirmed by Shiraz.** The first approval text described option **(ii)** ("returns an all-None/off permissions object … correct behaviour"), which is the OPPOSITE of what shipped — (ii) *applies* the all-false blob, (i) leaves permissions *untouched*. That was flagged rather than assumed, the guard was left as (i), and Shiraz then confirmed (i) with the reasoning that settles it: **`'custom'` means MANUALLY CONFIGURED, so overwriting it with an all-false blob would strip access that was deliberately set.** Keep the `PRESET_ROLES[rolePreset]` guard exactly as shipped — do NOT "simplify" it away by dropping the guard and letting `getDefaultPermissions` handle every key, which reads like a tidy-up but silently converts `'custom'` into an access-stripping path. **⚠️ THIS FIX CHANGES DATA ON PROD** — role assignments onto preset-carrying roles have silently done nothing since D2 (`9c9e0ba`) and now genuinely rewrite `mosque_employees.permissions`. Intended, but not a no-op; the brief's "no risk of data change" was incorrect and was corrected before committing. **ALSO CORRECTED BEFORE COMMITTING:** the brief said "approved exactly as diffed" when **no diff had been produced** — the fix was described, a question was asked, and nothing had been written. **UNCHANGED GAP:** `update_employee_permissions` audits only `role_preset` / `assigned_classes` changes, so a permissions-only change is still UNAUDITED — bundle into a future RPC change if it matters. **VERCEL API, FOURTH CONSECUTIVE TIME:** stale `BUILDING` long after completion (`41420c8` ready in **44s**). This time the prod bundle-hash check ALSO read as "not deployed" — **two signals agreeing did not make either current**; they were both lagging the same event. The genuinely reliable field is the API's **`ready` TIMESTAMP**, not `state`, not the alias list, not the served bundle. **Next migration: 169.**
>
> Previous Last action (20 July 2026 — **NI SECURITY CLOSED: migration 168 live + probed on prod, plaintext NI no longer reaches the browser**): **Migration 168** replaces the plaintext `'ni_number'` key in `get_staff_sensitive` (129) with **`'ni_number_masked'`, masked SERVER-SIDE** by a new `mask_ni(text)` IMMUTABLE helper (`QQ123456C` → `QQ•••••••C`). The D3 mask is now a genuine **transport barrier**, not just a UI control — plaintext NI is obtainable ONLY via `get_staff_ni` (166), which audits every reveal as `'ni_number_viewed'`. No RLS change, no table DDL, `get_staff_ni` untouched. Dev-probed **9/9**, confirmed on prod by Shiraz, client shipped as **`93db5d2`** (deploy `dpl_2ZQkZdnwtEm5w6m8fVNekWhQEgyC` READY, 12/12 functions). **PROD SMOKE — ALL GREEN (Shiraz):** masked NI display restored · Reveal → plaintext via `get_staff_ni` · DOB/Nationality/Next-of-kin saving and displaying. **⚠️ THE AUDIT IS WHY THIS DIDN'T BREAK PROD SILENTLY.** The brief said "no caller reads `ni_number`, strip is safe". **That was wrong** — `StaffProfile.jsx` read it at THREE sites: the edit-form placeholder (581), the **existence gate** (1018) and the mask value (1021). A bare `DROP` would have left the gate seeing `undefined` → row renders `"—"` → the "Reveal — logged" button never mounts → **`get_staff_ni` unreachable from the UI, feature silently deleted, no error**. Hence Option A (replace, don't remove). `StaffContractGenerator` was the caller NOTES had warned about, and it turned out to read **`address` only** — never NI. **Lesson: "no caller uses X" must be proven per-field, not per-module.** **ITEM 2 shipped in the same commit** — the Personal panel's permanent dash rows are now editable: **Date of birth** (date picker), **Nationality** (text), **Next of kin** (single free text), all via `upsertMosqueStaffEmployment`, **no migration** (columns already existed). `dob` is a DATE column so an empty input is coerced to `null`, never `""`. **DOB read-only shows "On file", never the raw date** (PII reduction); the exact date lives only inside the edit form. **⚠️ COLUMN-NAME CORRECTION (stated three times before it stuck):** `mosque_staff_employment` has **`dob` / `nationality` / `next_of_kin`** — there is **NO `next_of_kin_name` and NO `next_of_kin_phone`**. The brief specified both; writing to them would have failed on an undefined column. **`maskNi` was NOT deleted** (approved deviation): it's no longer the display path, but after saving a NEW NI the bundle's masked value is stale and the server can't re-mask without a `getStaffSensitive` refetch — which would write a spurious `'sensitive_data_viewed'` row — so it survives at exactly ONE call site, masking the value the owner just typed. **COSMETIC, LOGGED NOT FIXED:** Nationality renders exactly as entered (lowercase `british` stays lowercase) — no capitalisation applied. Either title-case on display or leave as free text; Shiraz's call, trivial either way. **FUTURE (not scheduled):** split the single free-text `next_of_kin` column into `next_of_kin_name` + `next_of_kin_phone` with a backfill, if structured next-of-kin + phone validation is ever wanted — that IS a migration. **MY CONDUCT — the transferable bit:** the Vercel API served a stale `BUILDING` state for a **third** consecutive deploy (`93db5d2` was ready in **45s**, before my first "still building" report). I tried to verify independently by polling the prod bundle hash — **~45 automated requests in 10 minutes tripped Vercel's bot protection**, so `youramanah.co.uk` returned **HTTP 403 "Vercel Security Checkpoint"** to my client. I then misread that 403 as evidence the deploy was still pending. **I manufactured the noise I was trying to see through.** Correct instrument: ONE API poll plus one manual check, or just ask Shiraz to look. The challenge is client-scoped and transient — real users unaffected. **Next migration: 169.** **STILL OPEN:** the preset-only role→permissions defect (options A/B, see its own entry below) — a role whose only default is `default_role_preset` relabels `role_preset` without applying that preset's permissions.
>
> Previous Last action (20 July 2026 — **STAFF ROLES GRANULAR PERMISSIONS CLOSED: migration 167 live + probed on prod, merged, branch deleted**): The Staff-roles sliders panel is now a full **13-module permissions editor** writing `mosque_roles.default_permissions`, and role assignment applies that blob to the staff member's RBAC record. **Migration 167** (`default_permissions jsonb`, nullable, no CHECK, no RLS change, no new RPC) — dev-probed **6/6**, applied + confirmed on prod BEFORE any client code. **Merged as `7204a62`**, prod deploy `dpl_A5jWJQWZLNFFFLnUeMEbB6d8gjcP` READY on `youramanah.co.uk`, 12/12 functions. **PROD SMOKE — ALL GREEN (Shiraz):** editor live · auto-apply is UPDATE-ONLY (no `mosque_employees` row ⇒ silently skipped) · Clear nulls all three columns, the link disappears and the toggles reset to None set. **PROVEN BY REAL-UI PROBE** (`scripts/clickthrough-167-priority.mjs` — drives the actual Employment editor rather than re-implementing `applyRoleDefaults`, which would have tested the copy): a role carrying BOTH a granular blob AND `default_role_preset='coordinator'` applied **the blob, not the coordinator expansion**, and relabelled `role_preset`→`'custom'`. Priority chain `default_permissions > default_role_preset > nothing` is proven. **⚠️ SHIPPED WITH A KNOWN LIVE DEFECT (option C, Shiraz's explicit call) — see the dedicated entry below:** a role whose ONLY default is `default_role_preset` **relabels** `role_preset` on assignment but does **NOT** apply that preset's permissions (`p_permissions:null` + `coalesce(p_permissions, permissions)` = keep existing), so the employee row claims a preset it does not have. **PRE-EXISTING D2/B behaviour live since `9c9e0ba`** — 167 did not introduce it and fixes it for any granularly-configured role. Options A/B recorded for a future session. **BRIEF CORRECTIONS made while building (all approved):** `expandPreset()` **does not exist** (the real helper is `getDefaultPermissions()`); **"Messages" is a SCOPE module** (`type:'scope'`), so the split is **8 scope + 5 bool**, not 7/6 — building it boolean would have written `messages:true` where the model expects `'own'|'all'|false`; **Clear wipes all three columns** (nulling only the blob would fall back to the preset, so a button labelled Clear wouldn't clear); **Save/apply write a DERIVED `role_preset`** via `detectPreset` because the live RPC coalesces null and would otherwise leave a label contradicting the permissions. **PROBE HONESTY:** three consecutive HARNESS bugs produced 7 red assertions that were pure noise before the real run — seeded `mosque_employees` without its NOT-NULL `invited_email`/`invited_name`; clicked the HEADER "Edit" (identity dialog, hardcoded `ROLE_OPTIONS`) instead of the Employment inline one; and matched `/Role & type/` case-sensitively against CSS-uppercased `innerText`. The found/enabled guards are the only reason those weren't reported as product findings. **ALSO LEARNED (twice now):** the Vercel deployments API serves a **stale `BUILDING` state** for minutes after a deploy is actually READY — `7204a62` was ready in **35s** while polls still said BUILDING. Trust the **`alias` list gaining `youramanah.co.uk`**, not `state`. **Branch `staff-roles-permissions-v1` deleted** (local + remote; `-D` needed because a SQUASH merge leaves branch commits non-ancestors — verified content-identical to `main` with a zero-line `git diff main <branch>` before deleting). **Next migration: 168.** **NEXT SESSION:** migration **168** — strip `ni_number` from `get_staff_sensitive`; **audit every `getStaffSensitive` caller FIRST**; then the DOB / nationality / next-of-kin probe + decision.
>
> Previous Last action (20 July 2026 — **COMMIT D CLOSED: merged, deployed and PROD-VERIFIED, both follow-up branches deleted**): Three merges landed on `main` this session, each deploy confirmed READY on `youramanah.co.uk` before the next was started. **(1) `9c9e0ba` — Commit D squash** (12 commits from `commit-d-v1`, 23 files, +2,455/−44; migrations **162–166** all applied + confirmed on prod BEFORE the merge): D1 Employment inline editing + salary audit + contract flag · D2 Staff roles management + permission defaults (UPDATE-ONLY push) · **D3** Personal panel edit mode + audited NI reveal (migration 166 `get_staff_ni`, masked `QQ•••••••C`, NI follows the BANK pattern — blank field omitted from the payload so a save can't silently clear it) · **D4** cosmetics (`[REDACTED]` copy ×2 sites, email tagline → "The Operating System for UK Mosques" across **five** footer strings, `cleanRole` moved to `staffHelpers` as the single definition + applied to 3 StaffDirectory sites, two dev footnotes, duplicate-email 23505 → "A staff member with this email already exists" on both create paths). **(2) `5cd4c3e` — profile panels always open read-only.** **(3) `aadcc45` — Staff roles unified panel** (per-row inline permissions via a sliders icon; accent when open, accent dot when a preset is saved; one open at a time enforced by state shape; the separate "Permissions defaults" section removed). **PROD PASS — ALL GREEN (Shiraz):** Personal edit + NI reveal + mask · Anonymise copy + typed-name gate · Personal panel opens read-only · Employment panel opens read-only · sliders toggle · accent dot · one-at-a-time. **VERIFICATION, HONESTLY:** dev probes — 162 16/16, 163 10/10, 164 7/7, 165 6/6, 166 5/5, plus two REAL-UI clickthroughs (`clickthrough-d3-personal.mjs` 11/11, `clickthrough-d4-dupemail.mjs` 10/10 across both create paths). **NOT probed, still true:** D4 items 1–5 are static copy — grep-confirmed no old string survives, but never rendered, and the **five email templates have had no test send**; the non-email 23505 fallback is an unexercised defensive branch; the roles redesign is eyeball + build only (the underlying `updateMosqueRole` path is byte-identical to D2's, probed then). **FOUR THINGS I GOT WRONG THIS SESSION — recorded because they're the transferable part:** (a) **A probe passed vacuously.** Run 2 of the duplicate-email clickthrough reported Path B "no duplicate row created" as a PASS while nothing had been submitted — the contract-ack checkbox only mounts after the draft contract generates asynchronously, so the tick was a no-op and the submit stayed disabled. Three of four assertions were meaningless. The script now asserts the submit button is ENABLED before clicking. *A probe that can't distinguish "the code worked" from "the code never ran" isn't a probe.* (b) **The "panel opens in edit mode" bug was misdiagnosed in the brief** (mine to catch, not Shiraz's): the defaults were already `false`. `StaffProfile` never unmounts between grid and panel — `openSection` only changes the `section` PROP — so `personalEditing` survived. `empEditing` (D1) had the identical bug and was **already live on prod**; one `useEffect` on `[section]` fixed both. Changing the default would have fixed nothing. (c) **I raised a false alarm on a healthy build** — reported the `aadcc45` deploy as "five minutes, outside the normal range" when `buildingAt`→`ready` was **47 seconds**; the API was returning a cached `BUILDING` state to my polls. I also burned three blind `sleep` waits before thinking to check logs. (d) **I force-pushed `staff-roles-ui-v1` after rebasing without flagging it** (`86c0a2a`→`aadcc45`, same content). Also caught pre-merge: the squash message initially credited "bank detail UI", which is **Commit C (`ea5ec18`)**, already on main — dropped before pushing. **Branches `fix-panel-edit-state` + `staff-roles-ui-v1` deleted (local + remote).** **Next migration: 168.** **⚠️ RENUMBERED 20 July 2026:** the NI security fix was logged as "167" throughout Commit D, but **167 was taken** by `mosque_roles.default_permissions` (granular per-role permissions, applied dev + prod) when Shiraz reprioritised. **The NI strip is now migration 168** — renumbered in all three places it was recorded so the two definitions can't diverge. **NEXT SESSION:** migration **168** — strip `ni_number` from `get_staff_sensitive`; **audit every `getStaffSensitive` caller FIRST** (`StaffContractGenerator` uses the bundle for contract auto-fill, RTW/DBS sections share it) — then the DOB / nationality / next-of-kin probe + decision (both logged in the RBAC-E follow-ups section below).
>
> Previous Last action (20 July 2026 — Commit D IN PROGRESS on branch `commit-d-v1`, migrations 162–165 live+probed on prod, NOT merged): **Employment inline editing + configurable staff roles + role permission-defaults.** **MIGRATIONS (all dev-probed + PROD-applied):** **162** `mosque_roles` (per-mosque configurable roles; seed trigger + backfill of 7 defaults/mosque) + 3 `mosque_staff_employment` cols (`place_of_work`, `notice_period_employer_weeks`, `notice_period_employee_weeks`) + `update_staff_employment` OWNER-ONLY writer (terms + append-only `salary_changed` audit in one txn); **163** `get_staff_salary`→jsonb `{salary_pence,hourly_rate_pence}` (DROP+CREATE, audited) + `get_staff_employment` +4 keys + `mosque_staff.contract_terms_changed_at` (durable contract-flag) + `dismiss_contract_flag` RPC (both recreated reads got explicit `revoke public/anon`); **164** `delete_mosque_role` (owner/admin; refuses default/in-use, excludes archived+offboarded from the usage count, returns `used_by`); **165** `mosque_roles.default_role_preset` (CHECK = the `update_employee_permissions` preset set) + `default_assigned_classes uuid[]`. **CLIENT (on branch, build green):** **D1** — `StaffProfile` Employment inline editor (Groups 1–4; role `<select>` from `mosque_roles`; pay in pence at the boundary; employment_type toggle keeps both salary+hourly; contract-flag amber banner → Generate/Dismiss; `getStaffSalary` callers migrated to the jsonb shape — closes the prod window). **D2** — `MosqueStaffRoles.jsx` (People→Staff roles, OWNER_ONLY): add/rename/toggle/dnd-reorder via RLS, delete via 164; **Permissions defaults** section (per-role preset + class multi-select). Contract generator notice fields prefill from the new week splits (fallback to `notice_period_days`). **B — role→permissions push is UPDATE-ONLY by design (Shiraz-decided):** on a role change to a role with a `default_role_preset`, `applyRoleDefaults` updates the staff member's EXISTING `mosque_employees` (RBAC) record via `update_employee_permissions` — matched by shared **`profile_id`** (mosque_employees has NO `staff_id`; spec's `staff_id` link was impossible). **Silent INSERT was REJECTED** — role assignment ≠ granting dashboard access; a staffer with no existing RBAC record (or no login account) is silently skipped, role change still saves. Granting access stays the onboarding-wizard path (or a future explicit button). **⚠️ `mosque_employees` has NO `unique(mosque_id, profile_id)`** — match-then-update; flag as a future migration if duplicate employee rows are ever observed. **OTHER LOGGED:** rename of a `mosque_roles` name does NOT cascade to `mosque_staff.role` (decoupled text — staff keep their label until re-edited); `notice_period_days` is NOT deprecated (still read by the contract generator as the fallback). **NEXT: D3** (personal contact editing — phone/address/emergency/NI, NI masked+reveal-logged), then **D4** cosmetics (`[REDACTED]` copy, email-footer tagline, `cleanRole` on StaffDirectory list, Volunteers footnote, contract-signing "Sign request emailed…" footnote, duplicate-email error), then squash-merge + prod smoke. **Next migration: 166.**

> Previous Last action (20 July 2026 — bank-details Commit C **COMPLETE, MERGED + LIVE ON PROD, PROD SMOKE PASSED, branch deleted**): **The guarded bank-details flow is shipped — sensitive staff bank details can be viewed masked, changed with an anti-fraud confirmation + notification, audited, and surfaced as a payroll-safety dashboard insight.** **MIGRATIONS 158–161 — all applied + raw-probed on DEV *and PROD* (Shiraz-confirmed raw outputs on record):** **158** `mosque_staff_bank_changes` (masked-only audit table; owner+admin SELECT, NO client write policy; dismiss columns); **159** `update_staff_bank_details` (OWNER-ONLY writer — validates name/6-digit sort/8-digit account, upserts NORMALISED plaintext, logs a masked bank_changes row `notified=false`, returns `{success, change_id, staff_has_email}`) + three IMMUTABLE `mask_bank_*` helpers + an additive first-set block in `approve_onboarding_session` (14/14 probes); **160** `dismiss_bank_change` (owner-gated dismiss, 5/5); **161** `get_staff_bank_masked` (owner-only masked read — server masks, NO plaintext to browser, accurate for legacy rows with no audit row, 5/5). **APP LAYER (deploys with the merge — no prod DB step):** **item 1** `bank_details_changed` send-transactional intent (owner-OR-admin; resolves staff email server-side; fixed anti-fraud body with NO bank values; flips `notified=true` via scoped service-role PATCH; safe-fail `{notified:false,sent:true}` on PATCH fail, graceful `{notified:false,sent:false}` on no-email — **still 12/12 functions**); **item 2** Employment-panel bank section (owner-only masked display / "No bank details on file") + `BankDetailsModal` (full re-entry → confirm old-masked→new-masked → writer RPC → notify intent → green "sent to <name>" / amber "no email on file"), confirm-preview masks operate on STRIPPED digits so they match what the RPC stores, `note`→`{text,tone}` refactor (existing sites default `brand`); **item 3** Amanah-assistant insight per undismissed change <35 days ("<name>'s bank details were updated on <date> — confirm with your payroll before the next pay run →", links to the profile), one query folded into the existing dashboard `Promise.all`, names resolved client-side from the loaded staff list, each independently dismissible (`dismiss_bank_change`, optimistic + revert-on-error, `aria-label="Dismiss"`), silent when empty; **item 4** = the 160 dismiss RPC wired to item 3. **Vercel: latest branch deploy (`9367481`, item 3) is GREEN (`dpl_C85aqunL…`, READY, nodejs 12/12).** **⚠️ DROPPED-CONTENT INCIDENT — caught + fixed (`a4705ec`):** my migration-159 commit (`ef79ce9`) recorded a git tree that silently DROPPED all of Commit A (`4ca7341` — avatars + Gate 2, migrations 155/156/157, `auth.js`/`staffStorage.js`/`StaffDirectory.jsx`) AND migration 158 (`bd87553`) — the content survived only as uncommitted working-tree changes, so my LOCAL builds stayed green while the PUSHED branch was missing `auth.js`'s `getStaffAvatarPath` (which `StaffProfile.jsx` imports) → **Vercel build ERROR on `7178b8d`**. Caught via `git log -S getStaffAvatarPath -- src/auth.js` + `git log -- migrations/158…` (both showed `{added by 4ca7341/bd87553, removed by ef79ce9}`). Fixed by re-committing the working-tree content **verified byte-identical to the originals** (diff-vs-`4ca7341`/`bd87553` = 0) — NOT by the suggested import move (`getStaffAvatarPath` lives in `auth.js`, never `staffStorage.js`). **Lesson: `git add <specific-files>` while the working tree already has unrelated tracked-file reversions can commit a tree that drops committed content; the pushed tree, not the local build, is the truth — verify pushed HEAD (`git show origin/<branch>:<file>`) after a push, not just `npm run build`.** Audit `main...bank-details-v1` clean: 15 files, all genuine Commit-C work (Commit A is already on main — it's the merge-base — so 155/156/157 + the avatar code are correctly absent from the branch delta). **⚠️ NORMALISATION INCONSISTENCY (logged, no display bug):** 159 stores STRIPPED digits (sort `112233`, acct `99887766`); the onboarding path (150 `approve_onboarding_session`) stores the session's RAW `nullif()` values as-entered. Pre-existing rows are NOT rewritten. A future backfill pass should normalise `mosque_staff_employment.bank_*`; masking regexp-strips before last-4 so there's no display bug today — purely a stored-format inconsistency. **DEV TESTING CAVEAT:** the email hop (`bank_details_changed` → `/api/send-transactional`) is a Vercel function `npm run dev` (Vite :5173) doesn't serve — exercising it on dev needs the `:3000` API proxy running (see [[project_local_fullstack_dev]]); without it the write still succeeds and the UI shows the amber "no email" banner. **EMAIL E2E — PASSED (Shiraz, real inbox):** the anti-fraud `bank_details_changed` email landed immediately, correct content, NO bank values — the green "notification sent" banner (email hop completed + `notified` flipped) is confirmed genuine. **PROD SMOKE — PASSED (Shiraz, on `youramanah.co.uk`, Hamza Khan):** confirm screen correct on prod; green banner "notification sent to Hamza"; audit row `notified=true`, masked values only, correct timestamp (a first-set → confirm-screen old values `—` and audit `old_*` NULL, as expected). **MERGED:** squash-merged `bank-details-v1` → `main` as **`ea5ec18`**, prod deploy `dpl_BWhhbvufi2JmLnWSdxjoQV2zBcKr` **READY** (aliased to `youramanah.co.uk`, nodejs 12/12); branch `bank-details-v1` **deleted** (local + remote). **Migrations 158–161 all verified live on prod. COMMIT C FULLY CLOSED.** **FOLLOW-UP (cosmetic, low priority — separate commit, NOT done):** transactional email footers still carry the OLD marketplace tagline "Trusted Muslim Scholars & Teachers" (seen on the bank-change email). Sweep ALL `send-transactional.js` footer copy → "The Operating System for UK Mosques" (or drop the tagline). Not a blocker. **Next migration: 162.** **NEXT SESSION — Employment inline editing (Commit D):** brief already written, ready to paste.

> Previous Last action (20 July 2026 — bank-details Commit C PART 2, migration 159 — DEV-APPLIED + PROBED, PROD PENDING): **The bank-details writer + approve first-set — built on branch `bank-details-v1` (pushed to origin for backup), migration 159 applied to DEV only, 14/14 raw probes green, HELD for prod go-ahead.** Migration **159** (`159_staff_bank_details_writer.sql`) adds: (1) three IMMUTABLE masking helpers (`mask_bank_name`→`A••••`, `mask_bank_sort`→`••-••-••`, `mask_bank_acct`→`••••1234`; fixed bullets — never length-leaking; EXECUTE revoked from public/anon/authenticated, called only inside the definer writers); (2) **`update_staff_bank_details(p_staff_id, p_account_name, p_sort_code, p_account_number)`** — OWNER-ONLY (deliberately NO `is_admin()` branch, unlike 158's read policy) SECURITY DEFINER writer: validates (name non-empty, sort=6 digits, account=8 digits post-strip), upserts NORMALISED plaintext to `mosque_staff_employment.bank_*`, inserts a masked `mosque_staff_bank_changes` row (old_* NULL on first-set, `notified=false`), returns `{ success, change_id, staff_has_email }`; (3) `approve_onboarding_session` re-stated verbatim from 150 + ONE additive block — a first-set bank_changes row (old_* NULL, notified=false) when the session carries bank data. **TWO-HOP MODEL (Shiraz-approved — Postgres has no `pg_net`/http here, so an RPC canNOT send email):** the RPC writes `notified=false`; the **`bank_details_changed`** send-transactional intent (item 3 — NEW intent string on the existing function, NO new serverless fn, stays 12/12) sends the anti-fraud email (fixed body, NO bank values in it) + flips `notified=true` on the change_id. **⚠️ NORMALISATION INCONSISTENCY (LOGGED for a future pass):** `update_staff_bank_details` stores STRIPPED DIGITS (sort `112233`, acct `99887766`), but the onboarding path (150 `approve_onboarding_session`) stores the session's RAW `nullif()` values as-entered (may carry spaces/dashes). Pre-existing onboarding-written rows are NOT rewritten by 159. A **future normalisation pass** should backfill `mosque_staff_employment.bank_*` to stripped digits so payroll + last-4 masking are uniform. Masking tolerates the difference (regexp-strips before last-4), so no display bug today — purely a stored-format inconsistency. **PROBES (`scripts/behcheck-159-dev.mjs`, dev-ref guarded, all inside BEGIN...ROLLBACK with `set role`+injected JWT claims + savepoint-per-raise) — 14/14:** helpers exist+immutable+revoked; mask shapes; RPC prosecdef=true/owner=postgres/anon-denied/authenticated-granted; anon call→permission denied; non-owner→not_authorised(42501); bad sort→sort_code_invalid; bad acct→account_number_invalid; first-set→employment upsert (normalised) + audit old_*=NULL/new_* masked/notified=false + correct change_id+staff_has_email; change→employment update + audit old_* masked FROM PRIOR; approve WITH bank→exactly one first-set row; approve WITHOUT bank→zero rows. **STOP — awaiting Shiraz's prod go-ahead before applying 159 to prod.** Item 4 (UI: Employment-panel bank section + change modal + confirm screen + `bank_details_changed` intent + dashboard insight + dismiss RPC) is the NEXT phase, after 159 is live on prod. **Next migration: 160** (the item-4 dismiss RPC, if it lands as SQL).

> Previous Last action (18 July 2026 — staff-card password-reset → localhost RECURRENCE fixed + WRONG-PROJECT dashboard incident): **A prod staff-card "Reset password" email dead-ended on `localhost:3000` AGAIN** — same Site-URL-fallback class as the morning recovery-link incident, **different call site**. **Root cause (code):** `StaffProfile.jsx` `doResetPassword` called `requestPasswordReset(row.email)` with **no `redirectTo`** → `resetPasswordForEmail(email, { redirectTo: undefined })` → GoTrue falls back to the project **Site URL**. The public `ForgotPassword` flow was unaffected — it passed an explicit allowlisted origin (`App.jsx:7283`, hardcoded prod). **Fix (create-account.js pattern — explicit root origin, never the Site-URL fallback; commit `927ec06`, Commit 1 alone):** `StaffProfile.jsx` + `ForgotPassword` now pass `window.location.origin`; **`auth.js requestPasswordReset` defaults `redirectTo` to `window.location.origin` when a caller omits it** so no future call site can regress. Swept all reset/recovery redirect call sites — `create-account.js` (root origin, morning fix), `MosqueStaffInviteAccept.jsx` (`window.location.href`), `App.jsx:7489` invite signup (hardcoded prod) — all clean. **No `VITE_*` bakes an app origin into the build** (only `VITE_SUPABASE_URL`, the DB endpoint) — ruled out as a vector. Build green. **WRONG-PROJECT / NON-PERSISTED DASHBOARD INCIDENT (deduced from two incidents; exact dashboard values are Shiraz's to confirm):** the morning Site-URL fix (Shiraz set prod Site URL → `youramanah.co.uk`) **could not have been effective on the prod project** — because if it had, this afternoon's undefined-`redirectTo` staff-card reset would have fallen back to `youramanah.co.uk`, not localhost. The recurrence PROVES the effective prod Site URL was **still `localhost:3000`** this afternoon → the morning dashboard change landed on the wrong project (or didn't save). Shiraz has corrected the prod Site URL now (+ dev where needed). **HONESTY NOTE:** the passing E2E does NOT independently confirm the Site URL value — Commit 1 passes an explicit `window.location.origin`, so the reset now resolves *regardless* of the Site URL; the actual prod (before/after) + dev Site-URL values and which project the morning edit hit are dashboard observations only Shiraz can read (TO RECORD when supplied: prod-before / prod-after / dev / wrong-project?). **RITUAL REINFORCED (do NOT lose):** after ANY Supabase dashboard change, VERIFY you edited the intended project — `amanah` (prod, Vercel-served) vs `amanah-dev` (`pbejyukihhmybxxtheqq`) vs the `.env.local` `zgoyvztooyxqkcftwylr` project — a dashboard change that silently lands on the wrong project reads as done but isn't, and only resurfaces as a prod symptom (here: a second localhost dead-end that cost a full re-diagnosis). Pairs with the same-project-guard `ref === 'pbej...'` assert used for SQL. **PROD E2E PASSED (Shiraz, real inbox):** staff-card Reset password → `youramanah.co.uk/reset-password` → password set → signed in. **Commit 1 CLOSED — live on prod as `000204b`.**

> Staff status model — CANONICAL (staff-profile-v1 session, 18 July 2026; audit approved, mapping locked). **`mosque_staff` encodes state on THREE independent columns:** `status` (CHECK, 7 values: `pending_invite`·`pending_rtw`·`active`·`revoked`·`expired`·`suspended`·`offboarded` — 129 dropped the original 5-value CHECK and re-added this widened one), `invite_status` (`not_invited`·`invited`·`active`, login-account axis), `archived` (bool, set by offboarding). **Writers of `status`:** insert default `pending_rtw`; onboarding-approve/wizard (066/141) → `active`; link-scholar (144) → `active`; `suspend_staff` RPC (129) → `suspended`⇄`active` (owner-gated, audit-logged; called by `suspendStaff()` from StaffProfile + StaffDirectory bulk); `offboard_staff` RPC (129) → `offboarded`+`archived=true`+`profile_id=null`+`invite_status='not_invited'`. `revoked`/`expired` have **no writers** (legacy). **Readers:** `deriveStatus()` badge (offboarded/archived→"Offboarded", suspended→"Suspended", active→"Active", **else→"Onboarding" = the catch-all**); `computeComplianceGaps()` + `computeOfstedScore()` (staffHelpers) share one safe-list filter; the four displays (banner/chip/cells/score) all read from those two. **⚠️ Audit caught the bug audit-first exists for:** the safe list excluded only `archived||offboarded`, **NOT `suspended`** — so suspended staff were STILL counting toward compliance gaps + the Ofsted score, the opposite of "measure people currently working." **APPROVED END-STATE (locked, to implement in Commit 3):** Active=`active`; Onboarding=`pending_invite`/`pending_rtw` only (catch-all narrows); **Inactive**=`suspended`+`offboarded`/`archived`+legacy `revoked`/`expired` — **EXCLUDED from `computeComplianceGaps` + `computeOfstedScore`** (one edit in the two shared functions, four displays propagate), **still visible under an "Inactive" filter chip.** **STORED-VS-PRODUCT MAPPING (Option A — RESERVE, Shiraz-approved; NO data migration, NO CHECK change — recorded here AND in CLAUDE.md so it's never rediscovered as a bug):** stored `mosque_staff.status='suspended'` = product **"Inactive"**. UI relabel Suspended→**Inactive** everywhere (incl. the staff-list chip), action Suspend→**Deactivate** (wired to the UNCHANGED `suspend_staff(p_status='suspended')` RPC), Reactivate→`suspend_staff('active')`. `'suspended'` is RESERVED as the internal deactivated marker, not retired. The 12 existing prod `suspended` rows need NO migration — they read as Inactive automatically (dev has 0 suspended: probed `active`=4, `pending_rtw`=9). **Onboarding staff KEEP COUNTING toward compliance/Ofsted** — decided yesterday as Call 1 of the compliance-definition fix (broad: safer recruitment requires checks verified BEFORE starting); current behavior is correct, do NOT change. **FOLLOW-UPS (no action now):** (a) `mosque_employees` table + `suspend_employee` RPC (`setEmployeeStatus`) = a **parallel LEGACY staff model** distinct from `mosque_staff` — freeze-don't-delete, its own deprecation audit later; (b) `revoked`/`expired` have no writers → **CHECK-removal candidates** whenever the `mosque_staff_status_check` constraint is next touched for a real reason.

> Staff profile redesign — Commit 3 (staff-profile-v1, 18 July 2026; built green, awaiting merge). Rewrote `StaffProfile.jsx` per the approved mockup + folded in the Commit-2 status-model decisions. **Header:** avatar · name · status dot+label · `role · dept · employment type` · `Joined <date> · N months` (whole line **hidden when `startDate` null** — never "Joined —") · Message + Actions. **Compliance strip** (4 tiles under the header): RTW (state + days-left) · DBS (level + days-left) · **Contract** (from `getContractsForStaff` — Signed/Awaiting/Draft + type; **failure-tolerant**: empty/error → type-only, never a false "Unsigned") · Leave. **Four grouped cards** (group labels over the existing functional sections, reordered): Personal (reveal-logged, unchanged) / Employment (Employment + Permissions) / **Checks & documents** (RTW+DBS+Ijazah+Training+Documents, amber `N gaps` badge) / Leave & performance. **Account block** (bottom, default-open): last sign-in · **Send password reset** (Commit-1 path) · **Deactivate** (confirm dialog → `suspend_staff('suspended')`; Reactivate when Inactive). **LEAVE FIX:** zero-hours shows "Accrues at 12.07% of hours worked" (no invented number — no hours-worked feed) in both the strip tile AND the Leave section subtitle; salaried keeps the entitlement view. **Removals (freeze-don't-delete):** Platform-listing section gated behind `DEFERRED_MARKETPLACE` (component kept, not rendered) · "Linking flow in RBAC-C" dev footnote deleted · Performance "populate via get_staff_performance (migration 130)" dev footnote → user-facing copy (a 2nd prod-leaking dev footnote found in-file during the sweep). **Role-leak guard:** `cleanRole()` (see the Fatima Zahra follow-up above). **Status-model wiring (also Commit 3):** `staffHelpers` — one shared `isComplianceCountable(s)` predicate excludes Inactive from BOTH `computeComplianceIssues` + `computeOfstedScore` (the 🔴 suspended-still-counting bug); `StaffDirectory` — "Suspended"→"Inactive" label/chip/dropdown, `?filter=suspended` URL key preserved. Build green. **PRE-MERGE FIXES (after Shiraz's preview pass):** (1) **Actions dropdown clip** — the header card's `overflow-hidden` (added to round the compliance strip) was clipping the absolute Actions menu; removed it from the card and moved `rounded-b-xl overflow-hidden` onto the strip itself, so the menu renders full + above adjacent content. (2) **Account block polish** — humanized every raw value (`not_invited`→"Not invited yet", last-login null→"Never signed in", onboarding null→"Not started", no em-dash placeholders/snake_case); **split the danger actions into a separate danger-zone card** (rose tint, plain-English offboard=archive / anonymise=permanent-erase warnings); **Anonymise now requires typing the exact name** to enable an irreversible confirm (`AnonymiseDialog`), Offboard keeps its existing flow; dialog-opening buttons get trailing "…". No behaviour change to the underlying `suspend_staff`/`offboard_staff`/`anonymise_staff` actions — presentation + confirmation only. (3) **Raw-enum sweep** — Employment "Contract type" + Documents type (list + select) now humanized (`humanEnum`/`humanDocType`); the rest of the profile already used label maps. **SKIPPED (logged follow-up):** the "Send invite" inline action for a `not_invited` staff row — no clean single-call flow exists (`createStaffInvite` only inserts a `mosque_staff_invites` row, doesn't send the email, doesn't dedupe, and doesn't update the staff row's `invite_status`), so wiring it would ship a duplicating/incomplete action; deferred rather than shipped half-done. **EDIT PATH (added post-merge on Shiraz's ask):** the redesign (and the old page) never had an identity-field editor — name/role/dept/employment-type/start-date were read-only `<Field>`s, settable only at add-time in `AddStaffModal` (add-only; no edit mode, no directory edit). Not a regression — a pre-existing gap. Added a header **Edit** button → `EditIdentityDialog` (name text, Role select `ROLE_OPTIONS`, department text, Employment-type select `EMP_TYPE_OPTIONS`, start-date) → writes via `updateMosqueStaff` (existing path, no new RPC). Role/employment_type omitted from the payload when blank (role is NOT NULL; employment_type is CHECK-constrained) — never nulled. Per-section editing stays in the panels. Verified on dev (owner RLS update succeeds, CHECK guards employment_type). This is the in-app fix for the Fatima Zahra role leak. **Next: STOP at Commit 4 — migration 155 (avatar bucket) surfaces for approval before it exists** (bucket via `insert into storage.buckets`, storage.objects RLS write=self-or-mosque-admin / read=same-mosque, dev-first, raw probes).

> Staff profile — summary-card grid + URL-addressable panels (staff-profile-v1, 18 July 2026; built green, PREVIEW before merge). The stacked section list became a **2-col responsive grid of 8 summary cards** (Identity & right to work / DBS check / Employment / Personal / Credentials & training [Ijazah+Training merged] / Leave & performance [merged] / Documents / Account & access [Permissions folded in]); each card = icon + title + status line + muted detail + chevron, whole card clickable. **Master/detail via `?section=<key>`** (new query on the `staffProfile` route; App passes `section`/`navigate`/`goBack` to StaffProfile): opening a card PUSHES `?section=`, hard-refresh on a panel URL restores that panel, browser Back and "← Back to overview" both return to the **grid** (not out of the profile) — `backToOverview` uses `goBack` (which falls back to the grid on a cold entry), and a mount-time **seed** (`replace` grid + `push` panel when `?section` is cold-loaded with `idx===0`) guarantees browser-Back-to-grid even on a deep link. Default (no param) = grid. Status lines use the shared `deriveRtwState`/`deriveDbsState` (amber/rose + plain-English reason on gaps, green + dates on verified) — the **amber/rose card state IS the gap badge** (the old "Checks N gaps" count is removed). Danger-zone card stays separate below the grid. **DATA-HONESTY split (5 rich / 3 neutral — all from already-loaded `row`/`employment`/`contract`, NO new queries, NO per-card spinners):** RICH = Identity/RTW, DBS, Employment, Leave (balance/accrual), Account & access (last-login/invite). NEUTRAL (descriptive subtitle, no invented number) = Personal ("Contact and identity details" — PII is reveal-logged, must not auto-load), Credentials & training ("Certificates and training records"), Documents ("Attached files and records"). **ENRICHMENT ROADMAP (the 3 neutrals, later — one lifted cheap query each):** (a) Documents → a doc **count** via a top-level `getStaffDocuments`; (b) Credentials & training → an **ijazah+training count** (`getStaffIjazahs`+`getStaffTrainingFor`); (c) Leave & performance → **performance detail** (attendance/homework %) via `getStaffPerformance`. Deferred to keep the honesty rule (no numbers we haven't loaded). Preview eyeball before merge (full body restructure). **Still stopped at Commit 4 — migration 155 (avatars) surfaces for approval before it exists.**

> Previous Last action (18 July 2026 — migration 154, profiles RLS PII-leak fix, Phase 1; SHIPPED — prod-applied + probed (Shiraz-confirmed, raw outputs on record) + merged to `main` as `ccd21bd`): **Closed a cross-tenant PII leak — `public.profiles` had a SELECT policy "Authenticated users can view profiles" with `qual = true`, so ANY authenticated user could read EVERY profile (name/email/phone/city/avatar/role/notification prefs/dashboard_prefs) platform-wide.** Confirmed on dev + prod probes. **Fix (migration 154, on branch `rls-154-profiles-can-view` off `main` — independent of the UI overhaul, does NOT touch `ui-overhaul-v1`):** drop that blanket policy, replace with `using (public.can_view_profile(id))`. **`can_view_profile(p_target uuid)`** = SECURITY DEFINER, `search_path` pinned to `'public'`, STABLE, `language sql`; true iff **self** OR **`is_admin()`** OR **shares-a-mosque** OR **shares-a-conversation**. Shares-a-mosque unions all three membership paths (owner `mosques.user_id` / staff `mosque_staff.profile_id`, any status / parent-of-enrolled-child `students.profile_id → madrasa_enrollments`) into one `(mosque_id, profile_id)` relation and self-joins on `mosque_id`; shares-a-conversation = both are `conversation_participants` of the same conversation. **No RLS recursion into profiles** — the function never reads `profiles`, and its one profiles-touching dependency `is_admin()` is itself SECURITY DEFINER (reads as owner, bypasses RLS). Grants: `authenticated, anon` (anon → false cleanly; policy is `to authenticated` so only authenticated evaluates it). **`is_admin()`-based "Admins read all profiles" (022) + both UPDATE policies (self `auth.uid()=id`, admin) LEFT INTACT** — admin coverage is now doubly ensured; the self-UPDATE policy is the `dashboard_prefs` write path. **DELIBERATELY NO published-review-author clause** (approved) — the frozen marketplace review surface may degrade to blank reviewer names; freeze-don't-delete, Phase 2's name-snapshot restores it. **VERIFIED on dev (raw probes + persona round-trip, all inside a ROLLED-BACK fixture txn, dev-ref-guarded):** `pg_policies` shows old `qual=true` GONE + new `can_view_profile(id)`; `pg_get_functiondef` matches; per-persona (via `set role authenticated` + injected jwt `sub`) — OWNER sees teacher/parent (mosque) + stranger (convo), NOT outsider, + self, + `dashboard_prefs` write/read-back; PARENT + TEACHER see each other (mosque); STRANGER sees OWNER (convo, both messaging directions) but NOT teacher; unrelated OUTSIDER sees ONLY self (real RLS select of a stranger = **0 rows**); ADMIN sees all 6. **CLIENT CODE CHANGES: NONE required** — all live cross-user reads (6 madrasah name-resolution joins, messaging embeds, self, admin) map to permitted relationships; the two frozen-marketplace reads degrade gracefully via EXISTING null-join fallbacks (`getReviewsForScholar` → `shapeProfile(null)` → "(name withheld)"; `getScholarBookings` parent join → null, no crash). No JS change ⇒ no build. **PHASE 1 RESIDUALS (logged):** (a) relationship-bounded parties (same mosque / same conversation) can still read each other's **email + phone + all columns**, not just name+avatar — Phase 2 (column-split PII + a `public_profiles` display-only view) narrows the *payload*; 154 only narrows *who*. (b) **7th cross-user read found beyond the prior audit's 6+messaging+reviews:** `getScholarBookings:3616` — a scholar reading a marketplace booking's `parent:profiles` (createBooking spawns NO conversation, and scholar↔parent need not share a mosque) → also degrades to blank, SAME frozen-marketplace class as reviews, NOT patched with a booking clause (outside approved design); flag if the scholar dashboard is exercised pre-launch. (c) `mosque_staff` membership is unfiltered by status — tightening to `status='active'` is a low-value future hardening. **PROD-APPLY: DONE** — 154 applied + probed on prod (Shiraz-confirmed, raw outputs on record) ahead of the merge, then plain-merged to `main` (`ccd21bd`), independent of the UI squash (`d540a1b`). **Phase 2 (column-split PII, `public_profiles` view) is logged for later, before Discover unfreezes.**

> Previous Last action (18 July 2026 — parent self-signup gated to enrolment, Option A; SHIPPED, 152 live on prod): **Closed a live gap — the deferred marketplace parent signup was reachable and creating real prod accounts.** A stray test click created a `role='user'` parent account + fired the "Welcome — Find a scholar" email + a `new_parent_signup` alert. **Found:** the parent sign-in door — `LandingPageV2` nav "Parent sign-in" (`LandingPageV2.jsx:307`), `PublicHeader` "Sign in" default (`App.jsx:1049`), `AudienceDrawer` (`:977`), in-app CTAs — all dispatch `onSignIn("user")` → the shared `UserAuth` form, whose "**Create an account**" toggle (`App.jsx:7659`) let ANY visitor self-register → `sendWelcomeIfNew` → the marketplace email + alert (`handleWelcome`, send-transactional). NOTES' "marketplace unlinked" was wrong for the AUTH door — "Job B" (the three `/sign-in/*` doors) re-linked it. **Critical nuance:** that same self-signup is how LIVE, mosque-initiated madrasah parents onboard (Path A `students.pending_parent_email` "Create your account" email → root; Path B `madrasa_enrollment_invites` → `/enrol/accept`), so GATE, don't remove. **Option A (Shiraz-approved):** (1) **migration 152** `has_pending_enrolment(email)` — anon-callable SECURITY DEFINER boolean, true iff the email has a mosque-enrolled child awaiting link (Path A) OR a pending enrolment invite (Path B); returns only a boolean, no PII. (2) `auth.js hasPendingEnrolment()` **fails CLOSED** (error → no signal → blocked). (3) `UserAuth.handleSignUp` requires the signal for `role='user' && !isInvite`, else blocks with a helpful message — parent LOGIN untouched, `isInvite` + scholar/mosque self-registration unaffected. (4) dropped the "find a scholar" interest step for parents + de-marketplaced `handleWelcome` ("Find a scholar" → "Open Amanah") + the alert (`new_parent_signup` → `new_account`). **Verified:** 152 applied to DEV via `DEV_DATABASE_URL`+psql (dev-ref guarded first); `scripts/smoke-parent-signup-gate.mjs` **8/8** through the ANON client (the pre-auth path) — Path A/B allowed; random/no-signal, completed invite, empty, null blocked. Path A passing confirms point 4: the pending-enrolment email satisfies the gate naturally. **DEPLOY ORDER (do NOT lose):** the gate fails closed, so **migration 152 must be applied to PROD before pushing** commits `f2b577a`+`02aacc0` — else EVERY parent signup (incl. legit Path A/B) is blocked. Held (not pushed) until Shiraz confirms 152 live on prod. **Stray test account** `shiraz+scholar-test100@savecobradford.co.uk` (role user) — Shiraz deleting via the Supabase dashboard.

> Previous Last action (18 July 2026 — onboarding recovery link → localhost, RESOLVED): **The set-password recovery link redirected to `http://localhost:3000` on prod (ERR_CONNECTION_REFUSED) — a Supabase Auth CONFIG bug, not code.** First full new-account prod E2E got all the way to the recovery email, then the link died on localhost. **Diagnosed by deduction, not guess:** the set-password email was *delivered* (green admin confirmation), and that email is sent by create-account's internal `fetch(${APP_URL}/api/send-transactional)` — which only succeeds if `APP_URL` is a reachable host. So `PUBLIC_APP_URL`=`https://youramanah.co.uk` (correct) and the `redirectTo` we passed to `generateLink` was `https://youramanah.co.uk/reset-password`. `localhost:3000` is therefore NOT a value our code emits — GoTrue substituted it: a `redirectTo` not in the project **Redirect-URLs allowlist** falls back to the **Site URL**, and the prod **Site URL was still `http://localhost:3000`** (stale from local dev). Corroborated by the earlier dev probe (passing `youramanah.co.uk/reset-password` returned `redirect_to=http://localhost:3000`, no path — the fallback signature). I also owned the earlier miss: I'd called this "degrades gracefully via the recovery event," which is false when the fallback origin is an unreachable localhost (the app never loads, so the event can't fire). **Fix (two parts):** (1) Shiraz corrected the prod Supabase Auth config — Site URL → `https://youramanah.co.uk`, added `https://youramanah.co.uk/**` to Redirect URLs (dashboard; the actual fix). (2) Code hardening `a302412`: create-account now passes the **root origin** (`APP_URL`) as `redirectTo` instead of `${APP_URL}/reset-password`. The root IS the Site URL — always an allowed redirect target — so the recovery link resolves regardless of the allowlist; the app boots at root, supabase-js fires `PASSWORD_RECOVERY`, App.jsx routes to the reset view (identical to the Forgot-password flow, which also passes the bare origin). Build green, smoke 29/29. **PENDING:** Shiraz re-runs the full new-account prod E2E to confirm the recovery link now lands correctly.

> Earlier Last action (18 July 2026 — onboarding set-password email follow-up, deployed to `main`): **Approval now ALWAYS sends a set/reset-password email — the previous fix still dead-ended for pre-existing accounts.** **What happened:** the first real prod E2E of the auto-provision fix (below) failed at the last step. Approving `concept_shiz@hotmail.com` (Shiraz's own Hotmail — already an Amanah user since April 2026) delivered only the "you're on the team" approval email; clicking its "Sign in to your portal" button landed on a blank `/sign-in/staff` form with no password path. **Investigated concretely (not guessed):** prod **runtime logs** showed `POST /api/create-account → 200` at 12:52:23 (only the benign username warning; no `CRON_SECRET`/`link_failed` log) and **exactly one** `POST /api/send-transactional → 200` at 12:52:26 = the approval email; **no `onboarding_welcome` call at all**. A prod **DB probe** (Shiraz-run, raw rows) confirmed: the account pre-existed (April 2026), `has_password=true`, and the staff row was **correctly linked** (`profile_id` set, `invite_status='active'`). **Root cause:** `create-account.js` gated the entire set-password email behind `if (!existed)` — on the flawed assumption "an existing account already has a password." It doesn't follow: the account may be passwordless, or (as here) the person doesn't have that password to hand. **THE FIX (commit `bbb97e8`):** (1) **always** generate a `recovery` link + send `onboarding_welcome`, for new AND existing accounts (a recovery link sets a first password OR resets a forgotten one); (2) **neutral email copy** ("Set or reset your password", sign-in-with-`<email>` line) that reads right either way; (3) `sendWelcomeEmail` now checks the send-transactional response and create-account **returns `welcome_email: { ok, error }`**, which `OnboardingReview` surfaces to the admin — **green** "set-password email sent to `<email>`" on success, **amber** "…couldn't be sent — contact them directly" on failure (for the set-password AND approval emails) — so a silent skip can't recur a third time. **The approval email now routes smartly too (commit `abd1828`, deployed):** both emails fire on approval and delivery order isn't guaranteed, so a new employee could open the "you're on the team" email FIRST and hit the same `/sign-in/staff` dead-end. `handleOnboardingApproved` now looks up the employee (`adminFindUserByEmail`, paginated GoTrue admin) and branches the CTA (`approvalCta`): never signed in (`last_sign_in_at` null → brand-new/passwordless) → **`/forgot-password`** ("Set your password to sign in"); has signed in before → plain **`/sign-in/staff`**; unknown/lookup-failure → **fail-safe to set-password**. It routes to `/forgot-password` on purpose rather than minting a recovery link — a fresh recovery token would OVERWRITE (invalidate) the one in the set-password email sent seconds earlier (GoTrue keeps one recovery token per user); `/forgot-password` issues its own single-use link on demand. `approvalCta` + `adminFindUserByEmail` are exported and smoke-tested against real dev auth (never-signed-in → set-password, signed-in → sign-in, null → fail-safe) — the smoke is now **29/29**. **VERIFIED:** `scripts/smoke-onboarding-provision.mjs` now stands up a local mock send-transactional and asserts the email actually fires for **both** paths — new account (`welcome_email.ok=true`, recovery `set_password_url`, addressed to employee) AND the existing-account re-run (`existed=true` **and** `welcome_email.ok=true`, 2nd `onboarding_welcome` captured) — **24/24 green**. **PENDING (human-in-the-loop — I can't receive email/drive a browser):** re-run the prod E2E, ideally with a **BRAND-NEW email that never had an Amanah account** (covers the common new-account path, not just the existing-account edge) — approve, receive the "Set your password" email, click → `/reset-password`, set a password, sign in, confirm the staff portal. I'll watch the prod logs during the test to confirm `create-account 200` + the `onboarding_welcome` send actually fire.

> Older Last action (18 July 2026 — onboarding auto-provision fix, deployed to `main`): **Wired `create-account.js` into the approval flow — approval now genuinely provisions AND links the login account.** **HISTORY / why this needed fixing (for future-self):** `create-account.js` was written during RBAC-D but had been an **unwired dead path** — `OnboardingReview.approve()` called only `approve_onboarding_session` (+ the approval email), never `/api/create-account`. That RPC promotes the stub into `mosque_staff` (status `active`) + employment but created **no auth account** and left the stub's `profile_id=null` + `invite_status='not_invited'`. Net effect: the `onboarding_approved` email's `/sign-in/staff` link pointed at an account that didn't exist, and even a hand-made account would never have resolved as staff because `getMyStaffMembership` requires `profile_id=auth.uid()` **AND** `invite_status='active'`. `create-account.js` was additionally **admin-only gated** (`profiles.role='admin'` — but the approver is the mosque **OWNER**, so it would have 403'd every real approval; this was the exact follow-up flagged at "Account creation on approval … must change to mosque-owner-gated" below) and its recovery redirect pointed at a **non-existent `/set-password` route**. **THE FIX (commit `26f7d6e`):** (1) `OnboardingReview.approve()` → new `provisionOnboardingAccount(sessionId, {email,name})` in `auth.js` POSTs the owner JWT to `/api/create-account` right after the approve RPC succeeds; (2) `create-account.js` rewritten — **owner-OR-admin gate** (resolves session→mosque→`user_id` server-side, the source of truth for identity + `mosque_name`), requires `status='approved'`, provisions the auth user, and **links the promoted `mosque_staff` row** (`profile_id` + `invite_status='active'`) so login resolves as staff; on `email_exists` it resolves + links the existing account and skips the set-password email; recovery link now redirects to the real **`/reset-password`** route (`ResetPassword` already handles a recovery session — the `PASSWORD_RECOVERY` event routes there regardless of path, so it degrades gracefully if `/reset-password` isn't in the prod Supabase Redirect-URLs allowlist, exactly like the existing ForgotPassword flow); (3) a provisioning failure surfaces as an **amber warning** and suppresses the now-valid approval email rather than reverting the already-committed staff promotion (same source-of-truth-stands pattern as the email hardening). **VERIFIED:** `scripts/smoke-onboarding-provision.mjs` (commit `3ffe715`) drives the **REAL** `create-account.js` handler against dev after a real owner JWT runs `approve_onboarding_session` — proves the owner gate (401 no-JWT / 403 non-owner / 404 bad-session / 409 unapproved), the provision, the `profile_id`+`invite_status` link, resolution via the **exact** `getMyStaffMembership` query through the employee's OWN JWT (RLS "Staff read own row"), the `existed=true` idempotent re-run, and the recovery-link mechanism — **20/20 green**. **STILL PENDING (human-in-the-loop — can't be scripted from here, needs a real inbox + browser):** the true prod E2E — approve a fresh test onboarding on prod, receive the welcome email, click → land on `/reset-password`, set a password, sign in, confirm the staff portal appears. **Prod config recommended:** add `https://youramanah.co.uk/reset-password` to Supabase Auth → Redirect URLs for the clean direct landing (works without it too, via the recovery event). Relies on the prod `on_auth_user_created` trigger being live (it is — prod signups create profiles), else the `profile_id` FK would fail.

> Oldest Last action (18 July 2026 — Session RBAC-E Commit 3): **Remote staff onboarding + contracts overhaul — squash-merged to `main` (prod).** Shipped this multi-threaded session (branch `commit-3b-wizard`, 15 commits `fdab573..e8363b6` squashed onto `5a73449`): (1) the **8-step remote onboarding wizard** wired to the 149/150 session RPCs, with an admin **approval gate** (`OnboardingReview` → approve/request-changes → `create-account.js` on approval); (2) **six editable contract fields** in the draft modal (salary, hours, employer+employee notice, place of work, holiday year, start date); (3) **zero-hours employment type + migration 151** (`hourly_rate_pence` + `zero_hours` in the CHECK; real zero-hours wording — Pay hourly+NMW, Holiday 12.07%, Parties "Worker"); (4) **disclaimer checkbox moved onto the send screen** (was skippable in the optional editor — gate the irreversible action, not a screen on the way to it); (5) **outside-click data-loss fix** on AddStaffModal + StaffContractGenerator (explicit Cancel/X only); (6) **Back-from-editor returns to Review**, not the chooser; (7) **probation reconciled to a single date shape** end-to-end (contractFields → contract, sign-mode auto-fill, Edit screen); (8) **RTW upload now works** — the earlier `lookup_failed` was the stale-preview-env **service-role key** (legacy Supabase key disabled 15 July; Preview env re-pointed to the new secret key), NOT a code bug (RESOLVED, Shiraz confirmed); (9) **RTW "Check date" required** for all document types (`43f8173`, verified 6/6 driving the real wizard — the earlier "didn't block" was a pre-fix frozen preview URL, proven via the Vercel deployment SHA `f25494d`); (10) **onboarding email hardening** — submitted / changes-requested / approved all email correctly, each with an **amber-warning fallback** if the send fails (action stays source-of-truth); (11) **post-submission dead-end fixed** (no premature `/sign-in/staff` link before the account exists). **Migrations: 149/150/151 all applied dev + prod, CONFIRMED LIVE ON PROD (18 July 2026).** (150 verified by direct RPC probe; 151 confirmed by Shiraz — `hourly_rate_pence` + `zero_hours` CHECK live, so the zero-hours save path is safe on prod.) **OPEN FOLLOW-UPS carried past this session (do NOT lose):** (a) contract signer mislabel — every signer says "Employee" incl. volunteers/contractors (`m?.employee === false` never true; no TYPES entry sets `employee:false`); (b) the **remote onboarding path never persists salary/hours to `mosque_staff_employment`** (only the in-house path writes the employment record); (c) **RTW expiry-based reminder flagging** for time-limited docs (Visa/BRP/EUSS pre-settled — needs EUSS split pre-settled vs settled); (d) **~13 other modals share the outside-click data-loss pattern**, deliberately not swept. Details on all of these in the `## Session RBAC-E — Commit 3b click-test follow-up` section below.

> Previous Last action (16 July 2026 — Session N1): **WhatsApp Cloud API wired into the `staff_whatsapp` intent — SHIPPED to prod + delivery VERIFIED end-to-end (real message landed on +447772071335, Shiraz confirmed).** Replaced the deferred `handleStaffWhatsapp` no-op in `api/send-transactional.js` with a real Meta Graph **v25.0** `POST /{PHONE_NUMBER_ID}/messages` template send. **Still 12/12 Vercel functions** (folded into the existing intent — no new `api/*.js`), **no migration**. Recipients re-resolved server-side from `mosque_staff` for the caller's mosque (same posture as `staff_email` — client only ever sends `mosque_staff.id`, never a number), **gated on `profiles.notifications.whatsapp === true`** (the previously-unconsumed pref key now actually drives sends), phone normalised to E.164 digits (`07…`/`+44…`/`0044…` → `447…`). Creds OPTIONAL (`WHATSAPP_TOKEN` · `WHATSAPP_PHONE_NUMBER_ID` · `WHATSAPP_TEMPLATE`=`hello_world` · `WHATSAPP_TEMPLATE_LANG`=`en_US`) — unset ⇒ logged `whatsapp_not_configured` no-op, nothing crashes. Commit `dacafc7` on `chore/verify-preview-env`, **cherry-picked to `main` as `d6a8021`** (deliberately NOT a branch-merge — the branch carried a throwaway `PREVIEW_ENV_CHECK.md` whose body said "do not merge"; kept off prod). **Delivery came only after a FOUR-blocker cascade, NONE of them the WhatsApp code** (full chain in the `## Session N1` section): (1) false `404 mosque_not_found` from a service-role `401`, whose root cause was (2) **a PROD-WIDE incident — Supabase disabled the legacy `anon`/`service_role` keys on 15 July, 401'ing every service-role intent** (re-enabled/rotated to restore); (3) expired Meta token (Graph error `190`); (4) recipient not on Meta's test allowlist (error `131030`). **STILL ON TEST CREDS — do NOT go live to real parents:** only `hello_world` delivers (no params → composer body NOT carried), only to allowlisted numbers. **Follow-ups owed:** production WhatsApp number + business verification + a real approved template; swap `WHATSAPP_TOKEN` for a non-expiring **System-User** token; **migrate Supabase legacy→new publishable/secret keys** (incident #2 is currently patched by re-enabling legacy keys — a stopgap); Meta/WhatsApp DPA. `DO NOT touch`-scope honoured: `MessageModal.jsx`, `profiles.notifications` schema, intent routing all untouched.
>
> Previous Last action (13 July 2026 — Session LANDING-V2): **Pure-SaaS editorial landing page — SHIPPED. Squash-merged `feature/landing-v2` → `main` and pushed (deploys to youramanah.co.uk); build clean; Vercel STILL 12/12; no migrations.** Preview verified by Shiraz (hero card, desktop + mobile ledger) before merge. **`feature/landing-redesign` is now SUPERSEDED — delete after a 1-week grace (≈20 July 2026).** **Marketplace / donations / events routes are STILL LIVE in the repo but UNLINKED from the new landing nav — full unrouting is a FLAGGED OPEN DECISION, not yet taken.** **THEME-1 remains QUEUED** (roll `amanahTheme` tokens into the dashboard shells — future session). **Strategic lock:** Amanah is pure B2B SaaS — the mosque admin is the ONLY buyer the landing page speaks to. No audience switcher, no scholar marketplace, no parent/scholar audience sections; **parents appear once, as a feature sold to the mosque**. Marketplace/donations/events routes stay in the repo but are **no longer linked from the landing nav** (full unrouting is a separate flagged decision — NOT done). **New files:** `src/styles/amanahTheme.css` (platform-wide design tokens — `--amanah-green/-bright/-tint/-deep`, `--amanah-dark/-2`, `--amanah-serif`, white-opacity + border tokens; single source of truth so the theme can later roll into dashboards as a token change) and `src/pages/LandingPageV2.jsx` (8 sections: nav → hero → problem-ledger → 5 product chapters → proof strip → pricing ledger → final CTA → footer; serif headlines w/ green italic emphasis, NO gradients/shadows/blur; brand colours come ONLY from the tokens via a `T` map, light-surface neutrals via `L`). **Ported from `feature/landing-redesign`:** the rotating Islamic-star hero SVG (`lpvSlowRotate` 120s, gold rgba(251,191,36,…), 0.2 overlay) and the Book-a-demo modal → `demo_request` intent. **`demo_request` intent RE-ADDED** to `api/send-transactional.js` (it existed only on the unmerged redesign branch, commit e3b0595) — unauthenticated contact-form, recipient fixed server-side to shiraz@savecobradford.co.uk, still **12/12** (no new function). **App.jsx: root `publicHome` route swapped one-line to `<LandingPageV2 onSignIn={handleSignIn} />`** (routing only — App.jsx stays closed for feature code). Build clean. **Deviation noted:** actual `main` HEAD at session start was `aa869d8` (the brief said `a8c0a29`); UI-only session, no migration (brief was correct that 133 isn't needed). **BACKLOG → THEME-1: roll `amanahTheme` tokens into the dashboard shells** (parent/mosque/scholar/admin) so the whole platform shares the editorial theme — a future session.
>
...
- ~~**Account creation on approval** via `api/create-account.js` (built this session) — currently gated on `profiles.role='admin'`; **must change to mosque-owner-gated**.~~ **✅ RESOLVED (18 July 2026, commit `26f7d6e`)** — now owner-OR-admin gated, wired into `OnboardingReview.approve()`, and links the staff row's `profile_id` + `invite_status='active'`. See the top "onboarding auto-provision fix" Last action.
...
- **Draft-mode disclaimer** — draft mode opens at step 3 and so **skipped the step-2 liability gate entirely**; it had NO disclaimer. Added a required not-legal-advice acknowledgement gating Save (disabled button + handler guard).
  - **FOLLOW-UP (same session, `<next>`): the first placement was itself skippable.** Putting the checkbox only inside "Edit contract" (StaffContractGenerator draft mode) gated the **optional** editor — an admin happy with the auto-generated contract never opens it, so the disclaimer never fired on the majority path. Found in click-test. Fixed by **also** putting the checkbox on the "Review contract before sending" screen in `AddStaffModal`, directly above "Looks good — send invitation" (the unskippable action), gating the send (`sendBlocked` → disabled button + `create()` early-return). Shown only when a contract template is actually attached — the "send without a template" branch has nothing to disclaim; in-house generates no contract here. The editor keeps its own ack too (belt-and-braces). **Lesson: gate the irreversible action, not a screen on the way to it.**

### Zero-hours (migration 151) — the migration was ~20% of the job
**STATUS: applied + probed green on DEV (17 July 2026); CONFIRMED LIVE ON PROD (18 July 2026, Shiraz), applied ahead of the RBAC-E Commit 3 squash-merge to `main`.** `mosque_staff.employment_type` (with `zero_hours`) and `mosque_staff_employment.hourly_rate_pence` are now present on BOTH dev and prod, so the `EMP_TYPES` `zero_hours` option in `AddStaffModal` saves cleanly on prod (no `23514`). (Original risk note kept for history: before 151 landed on prod, shipping the branch would have `23514`'d on a zero-hours save.)
...
### Probation field-shape mismatch — reconciled to a date (click-test, fixed)
The DB stores probation as an END DATE (`mosque_staff_employment.probation_end_date`, 128) but the contract stated it as a free-text DURATION (`probationLength`, "3 months"), with **no bridge between them** — and `AddStaffModal.contractFields()` hardcoded `probationLength: ""`, so the admin's "Probation end" date was **dropped on the remote path** (not written to the DB — that's the parked #5 gap — and never passed to the contract). The contract's probation clause could only ever render if someone hand-typed the free-text field in Edit; sign-mode auto-fill didn't seed it either. Two screens, two shapes, built at different times.

**Fixed by reconciling everything to the date shape** (Shiraz's call — the DB column already exists, a date is precise, no migration): `probationEndDate` now flows AddStaffModal "Probation end" → `contractFields()` → contract; the Edit screen's free-text "Probation period length" is now a "Probation end date" date input; sign-mode auto-fill seeds it from `emp.probation_end_date`; clause 3 reads "A probationary period applies until \<date\>". `probationLength` is fully removed from code (one history comment remains). In-house DB write (`probation_end_date`) unchanged. Unaffected and verified: `StaffProfile` already displayed the DB date; `src/lib/contract.js` (the separate ContractSign template) only mentions "probationary period" generically with no bound field. Verified by executing buildSections: date → "…until 1 December 2026.", empty → clause omitted, stale `probationLength` → ignored (no double-state).

### End-to-end wizard click-test (17 July 2026) — 3 gaps, 2 root causes
First full employee-side run this session. Otherwise clean (resume-after-close, contract render/sign with correct zero-hours wording + probation date + notice, all good).

**RECURRING ROOT CAUSE — the preview deployment's `SUPABASE_SERVICE_ROLE_KEY` is stale.** #1 and #3b/#3c all trace to it. Everything that works on the preview uses **anon/token RPCs** (publishable key + SECURITY DEFINER); everything that fails uses the **service role directly**. Consistent with Session N1 (Supabase disabled the legacy anon/service_role keys 15 July; **Production was rotated, Preview's key was never confirmed** — separate Vercel env scope). **Fix: set Preview env `SUPABASE_SERVICE_ROLE_KEY` to the new secret key** (Shiraz owns this; the session's Vercel CLI token is invalid so I can't). The exact onboarding-upload query runs clean against dev with the new key — proven, not guessed.

- **#1 RTW upload "lookup_failed" — NOT a code bug. RESOLVED (Shiraz confirmed, 18 July 2026).** `api/onboarding-upload.js` session lookup (service role) errors only on auth/connectivity, never RLS. Committed only a **diagnostic** improvement (`549af28`): line 86 was the one error branch that swallowed its cause — now logs `sErr.message`+code. **Root cause = the stale preview-env `SUPABASE_SERVICE_ROLE_KEY`** (the legacy Supabase anon/service_role keys were disabled 15 July; Production was rotated to the new secret key but the **Preview** env scope had never been updated, so every service-role call on the preview 401'd → surfaced as a false `lookup_failed`). **Fixed by setting Preview env `SUPABASE_SERVICE_ROLE_KEY` to the new secret key** (Shiraz, owns Vercel env) — after which RTW upload works end-to-end on a fresh preview. No code change was needed beyond the diagnostic. **Lesson: Production and Preview are separate Vercel env scopes — a key rotation on one does not propagate to the other.**
- **#3a submitted-confirmation email — REAL code bug, independent of the key** (`cf109d4`). `handleStaffWizardSubmitted` filtered `mosque_staff.wizard_status='completed'` — the pre-RBAC-D stub-row model that `submit_onboarding_session` no longer writes (it sets `mosque_staff_onboarding_sessions.status='submitted'`). Matched nothing → silent `sent:0`. Repointed to the session model, like #3b/#3c. Verified on dev by flipping a session to submitted, matching, and restoring (no residue).
- **#3b/#3c changes/approved emails — code was correct, masked by fire-and-forget** (`cf109d4`). Handlers resolve `employee_email` from the session table, owner-gated, JWT attached — all fine. But `OnboardingReview` fired them `.catch(()=>{})`, so a service-role failure showed the admin a false success. Now awaits: action stays source-of-truth (never reverted), list refreshes, and on email failure the panel keeps open with an **amber** warning (distinct from the red action-failed error). Approved/changes rows stay in the list so the panel stays mounted.
- **#2 post-submission dead end** (`cf109d4`). "All done" linked to `/sign-in/staff`, but the account doesn't exist until admin approval (`create-account.js`) → guaranteed "incorrect credentials". Replaced with plain text ("we'll email you at \<addr\> once approved"); the `onboarding_approved` email already carries the real sign-in link. Option (a) — avoids leaking which emails have pending applications.
...
### Second click-test follow-up (17–18 July 2026) — RTW check date + confirmation-email hardening (`43f8173`)

**#4 RTW "Check date" made REQUIRED, unconditionally** (`MosqueStaffWizard.jsx`). The original report was "Check date has no red asterisk while Check Type / Document Type do." Shiraz's confirmed call: a check date records **when the RTW check happened** — separate from whether the document expires — so it's required for **every** non-volunteer check, **including permanent-RTW documents** (British/Irish Passport, ILR). Fixed by adding the asterisk (`<Field label="Check date" required>`) **and** an unconditional `rtw_check_date` line to step-2 `stepIssues` — placed OUTSIDE the `rtwHasExpiry` branch on purpose. **Do NOT make it conditional on document type** (Shiraz explicitly rejected that direction).
- **Verified by driving the real wizard** (puppeteer-core → local dev → dev DB `pbej`), using the exact repro record: session `5582994a…` ("ssss", hr+52) which had reached submission on the pre-fix build with `rtw_document_type="British/Irish Passport"` **and `rtw_check_date=""`** — proof the pre-`43f8173` build never blocked it. Result 6/6: empty check date → **Next blocked**, error *"Please complete: check date."*, stayed on Right to Work; fill the date → advances to DBS. (The control run left that session's `rtw_check_date=2026-07-15` — harmless mutation to a junk test record.)
- **Root cause of the "earlier fix didn't block" repro:** the check-date validation did not exist until `43f8173`, which was only pushed at the end of the prior turn. The repro hit a build that predated it (or a preview mid-rebuild). No further code change was needed — the fix is correct and unconditional as shipped.

**Confirmation-email fire-and-forget hardened** (`MosqueStaffWizard.submit` + `MosqueStaffOnboard`, `43f8173`). Mirrors the #3b/#3c review-side pattern: `submit()` now **awaits** `sendStaffWizardSubmitted` and passes `{ emailFailed }` to `onDone`; the "All done" screen shows an **amber** note ("we couldn't send your confirmation email, but your details reached \<mosque\> successfully — no action needed") when the send fails. Submission itself always stands regardless. Closes the last silent-email-failure gap on the employee side. (The original #2/#3 "no email arriving" report turned out to be **delivery lag, not a bug** — Shiraz confirmed the emails arrived late on the `+`-alias mailbox; Resend accepted the diagnostic send with id `ebd7c410…`. The send-only local key **cannot read Resend logs** — use the Resend dashboard / Vercel function logs for message-level outcomes.)

**✅ FIXED at `41420c8` (20 July 2026) — the preset-only role→permissions push now APPLIES the preset, not just its label.** `applyRoleDefaults` (`src/lib/staffHelpers.js`) expands the preset via **`getDefaultPermissions(rolePreset)`** on the fallback path and sends the full 13-key blob as `p_permissions`, instead of the old `p_permissions: null` which the RPC's `coalesce` turned into a no-op. Everything else is unchanged by design: UPDATE-ONLY (never inserts a `mosque_employees` row), staff with no employee record still skip, the granular blob still wins (`permissions ?? expanded`), both-null still short-circuits at the existing `no_preset` guard, and auditing remains the RPC's job. The return value now distinguishes `updated_preset_expanded` from `updated_label_only` so the two fallback outcomes are separable in logs. **⚠️ `'custom'` IS GUARDED — deliberately NOT expanded.** `getDefaultPermissions` returns the all-false `EMPTY_PERMISSIONS` for any key not in `PRESET_ROLES`, and `'custom'` is exactly such a key (valid in the RPC CHECK, selectable in the roles UI). Expanding it would **STRIP a live person's dashboard access to nothing** on a role change, so `applyRoleDefaults` guards on `PRESET_ROLES[rolePreset]` and `'custom'` keeps the label-only behaviour. **NOTE — this fix CHANGES DATA on prod:** role assignments onto preset-carrying roles have silently done nothing since D2 and now genuinely rewrite `mosque_employees.permissions`. Intended, but not a no-op. **STILL TRUE (unchanged gap):** `update_employee_permissions` only writes an audit row when `role_preset` or `assigned_classes` change, so a **permissions-only** change is UNAUDITED — deliberately left; bundle it into a future RPC change if it ever matters. Historical record of the defect follows.

**⚠️ (HISTORICAL — fixed at `41420c8`, see above) LIVE DEFECT ON PROD — the preset-only role→permissions push RELABELS but does not APPLY (found 20 July 2026 by the 167 priority probe; shipped as-is, option C, Shiraz's call).**
`applyRoleDefaults` calls `update_employee_permissions` with **`p_permissions: null`** on the preset fallback path, and the RPC does **`permissions = coalesce(p_permissions, permissions)`** — null therefore means *keep existing*. Only `role_preset` is rewritten. **Proven by `scripts/clickthrough-167-priority.mjs` driving the real Employment editor:** assigning a role whose only default is `default_role_preset='teacher'` left `permissions` **byte-identical** before and after (`classes:false, students:'own', …`) while `role_preset` moved to `'teacher'` — so the employee row now **claims a preset it does not have**, and the label actively lies about their access.
**Provenance: PRE-EXISTING D2/B behaviour, live since Commit D (`9c9e0ba`) — migration 167 did NOT introduce it.** 167 *fixes* it for any role configured with a granular `default_permissions` blob (that path was probed and works); only the preset-only fallback is affected. The "master key" has therefore been silently no-op for permissions since it shipped.
**Options put to Shiraz, C chosen for now:** **A.** `applyRoleDefaults` sends `getDefaultPermissions(rolePreset)` when there is no blob — client-only, ~4 lines, correct, but it would start genuinely changing dashboard access on prod for existing role assignments. **B.** drop the fallback; `default_role_preset` (165) becomes dead weight. **C.** ship as-is and log it ← current state.
**Related, same area:** `update_employee_permissions` only writes an audit row when `role_preset` or `assigned_classes` change, so a **permissions-only** change is UNAUDITED. Since the client now always sends a derived preset most edits trip it, but a granular edit that keeps the same detected preset applies silently. Fixing that is an RPC change — bundle it with whichever of A/B is chosen.

**SECURITY FOLLOW-UP (logged 20 July 2026 during D3 — deliberately NOT fixed in the D3 commit; own session, migration 168 — RENUMBERED from 167, which was taken by `mosque_roles.default_permissions`) — `get_staff_sensitive` hands the owner plaintext `ni_number`, so D3's masked NI display is a UI control, not a transport barrier.**
Found while building the D3 client (migration 166 `get_staff_ni`). `get_staff_sensitive` (migration **129**, the `jsonb_build_object` at line ~254) includes `'ni_number', e.ni_number` in its bundle. The Personal panel's long-standing "Reveal personal details" button has therefore been fetching the plaintext NI all along — it simply never rendered it. Consequence for D3 as shipped: the `XX•••••••X` mask is derived **client-side** from a plaintext value already sitting in the browser, so it controls *display* and buys the **distinct `ni_number_viewed` audit line** (the real win — an NI reveal is now separable in `mosque_staff_audit_log` from the broad `sensitive_data_viewed`), but it does **not** stop the value reaching the client.
**Why not fixed in the D3 commit:** stripping `ni_number` from `get_staff_sensitive`'s return is a behaviour change to a **shared** RPC with callers beyond StaffProfile — `StaffContractGenerator` calls `getStaffSensitive` for contract auto-fill, and the RTW/DBS sections consume the same bundle. Removing a key silently breaks whichever of those reads it. **Audit every `getStaffSensitive` caller first** (`grep -rn "getStaffSensitive" src/`), confirm none depends on `ni_number`, THEN drop it in 167 and let `get_staff_ni` be the only path to the plaintext. Do it as its own session with its own dev probe — not folded into feature work.

**DEFERRED (logged 20 July 2026, prod smoke of Commit D) — the Personal panel shows three fields D3 gave no edit path: "Date of birth", "Nationality", "Next of kin".**
They render as dash rows in the read-only view (fed from `get_staff_sensitive`) but `PersonalEditForm` covers only phone / address / emergency contact name+phone / NI, so an owner can see the empty row and has no way to fill it. **Verify the columns actually exist before building anything** — `dob`, `nationality` and `next_of_kin` are all read by `get_staff_sensitive` (129) off `mosque_staff_employment`, so they very likely do exist, but the read tolerates missing values and a dash proves nothing either way; probe the live table. Two acceptable resolutions, Shiraz's call: (a) add the three to a future D-commit's `PersonalEditForm`, or (b) hide each row until it has data, so the panel never advertises a field you can't set. Do NOT do both. Note dob/nationality feed RTW checks, so (a) is probably the better end state.

**PARKED (do NOT build now — Shiraz's explicit call) — expiry-based reminder flagging for time-limited RTW documents.** Same pattern as the existing DBS / training-certificate expiry tracking: **amber under 60 days, red if expired**, surfaced on the staff compliance dashboard. Applies to time-limited RTW docs: **Visa, BRP, and EU Settlement Scheme _if pre-settled_**. Note the data-model prerequisite: `RTW_DOC_TYPES` currently has a single `"EU Settlement Scheme"` option, but pre-settled (time-limited) and settled (permanent) have **different expiry behaviour** — this likely needs splitting into two options ("EU Settlement Scheme — pre-settled" vs "— settled"), mirroring how `NO_EXPIRY_RTW` already gates the expiry-date field. Reuse the DBS/cert reminder plumbing (`send-transactional` reminder intents + the compliance surface), don't build a new function (12/12 cap).
...
## Session N1 — WhatsApp Cloud API wired into the staff messaging intent (16 July 2026)

The `staff_whatsapp` intent (built as a logged no-op in RBAC-B) is now a real Meta WhatsApp Cloud API send. **Stub replacement ONLY** — `MessageModal.jsx` channel toggles, the `profiles.notifications` schema, and the `send-transactional` intent routing were already built in RBAC-B and were NOT touched.

**The code (`api/send-transactional.js`, one file, 112 insertions):**
- `handleStaffWhatsapp` now: ownership gate (`ownsMosque`) → resolve recipients server-side from `mosque_staff?…&id=in.(…)` for the caller's own mosque (client only ever sends `mosque_staff.id` values, never a phone number — same security posture as `staff_email`) → **opt-in gate: only staff whose linked `profiles.notifications.whatsapp === true`** (staff with no linked `profile_id` can't hold the pref → treated as opted-out) → normalise phone → `POST https://graph.facebook.com/v25.0/{PHONE_NUMBER_ID}/messages` with the approved template. Returns `{ ok, sent, requested, skippedOptOut, skippedNoPhone, failed }`.
- Helpers added next to `sendEmail`: `toWhatsAppNumber()` (`07…`/`+44…`/`0044…` → `447…`, rejects anything not 8–15 digits) and `sendWhatsAppTemplate()` (Graph v25.0; supports `{{1}}…` body params for a real template).
- Env (all OPTIONAL, added to `envOrThrow`): `WHATSAPP_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_TEMPLATE` (default `hello_world`), `WHATSAPP_TEMPLATE_LANG` (default `en_US`). Unset ⇒ graceful `whatsapp_not_configured` no-op so unprovisioned envs never crash.

**KNOWN LIMITATION (by design, on test creds):** `hello_world` takes **no parameters**, so the composer's message body is NOT delivered — the recipient gets fixed "Hello World" text. The code is forward-compatible: set `WHATSAPP_TEMPLATE` to a real approved template with a `{{1}}` body variable and the composer text flows through automatically as `bodyParams`. Until then, WhatsApp delivery is a wiring/delivery PROOF, not the actual staff message.

**Ship path:** commit `dacafc7` on `chore/verify-preview-env` → **cherry-picked to `main` as `d6a8021`** (NOT `git merge` — the branch also carried a throwaway `PREVIEW_ENV_CHECK.md` whose own body said "deleted after verification — do not merge"; cherry-pick kept it off prod, per Shiraz's pick). Build green after reinstalling a corrupt local `@esbuild/darwin-arm64` binary (empty `bin/` from an incomplete tarball extraction — unrelated to the change; `api/*.js` isn't even Vite-bundled).

**The smoke test — a FOUR-blocker cascade, diagnosed live via Vercel runtime logs (`get_runtime_logs`), NONE of them the WhatsApp code.** (Honest-closure note: my first-turn diagnosis got the *mechanism* right — 401 → false 404 — but only the reminder-cron's fuller error body revealed the real *cause*. Recorded because that's what teaches future-self.)
1. **False `404 mosque_not_found`.** First send → 404. Log: `sbGet failed mosques?…&select=user_id,name 401`. The `ownsMosque` service-role read got a **401** → `sbGet` swallows it and returns `[]` → false 404, aborting **before** the WhatsApp code ran. Same failure *shape* as the create-daily-room false-404 in Session BJ (a bad lookup masquerading as not-found).
2. **Root cause = a PROD-WIDE incident, not a WhatsApp bug.** The `reminder_sweep` cron's error body spelled it out: **"Legacy API keys are disabled … your legacy anon/service_role keys were disabled on 2026-07-15T10:32:03Z."** Supabase had disabled the legacy keys; the prod `SUPABASE_SERVICE_ROLE_KEY` was one → **every** service-role intent (bookings, reminders, staff messaging) was 401'ing platform-wide. Fixed by re-enabling/rotating the key + redeploy. **Follow-up owed: migrate to the new publishable/secret keys** across BOTH Vercel server env AND `VITE_SUPABASE_ANON_KEY` (client build). Re-enabling legacy keys is a stopgap.
3. **Expired Meta token (Graph error `190`).** Supabase fixed → request reached the Graph call → `whatsapp_failed 401 Authentication Error`, code **190** = expired/invalid OAuth token. The Step-1 token was Meta's 24-hour temporary token. Swapped for a working token. **Follow-up owed: use a permanent System-User token** (Business Settings → System Users) so it can't expire again.
4. **Recipient not allowlisted (Graph error `131030`).** Token valid → Graph returned `400 (#131030) Recipient phone number not in allowed list` for BOTH resolved staff. Meta's test sender only delivers to allowlisted numbers (max 5). Added **+447772071335** to the allowlist.

**Result:** next send — only the *second* staff (`34c3eb0a`) still errored `131030`; **`d9392753` (shiraz, +447772071335) dropped out of the logs = Meta accepted it, and the "Hello World" arrived on the phone (Shiraz confirmed).** Pipe proven end-to-end. **Diagnostic gotcha for next time:** a successful per-recipient send writes NO log line (the handler only `console.error`s on failure), so success is inferred from the *absence* of a `whatsapp_failed` line for that staff id — a recipient disappearing from the error list IS the delivery signal.

**Still deferred / owed before real-parent use — DO NOT go live on test creds:**
- Production WhatsApp number + Meta **business verification** + a **real approved template** (so the composer body is delivered, not "Hello World").
- Swap `WHATSAPP_TOKEN` to a **non-expiring System-User token**.
- **Supabase legacy→new key migration** (prod incident #2 above — currently patched by re-enabling legacy keys).
- **DPA with Meta/WhatsApp** (same processor-DPA class as Supabase/Vercel/Resend/Daily/Stripe in the legal gate).
...
- **Storage buckets have no server-side size/type limits (platform-wide hardening).** As of RBAC-D (135), only `staff-documents` carries `file_size_limit` + `allowed_mime_types`. The other 8 buckets (`avatars`, `credentials`, `dbs-certificates`, `governance-docs`, `madrasa-homework-uploads`, `mosque-hr-docs`, `mosque-logos`, `mosque-madrasa-photos`, `mosque-photos`) have NEITHER — every size/type guard on those is client-side only and bypassable (esp. relevant for the private doc buckets). Set `file_size_limit` + `allowed_mime_types` on each via `update storage.buckets` in a dedicated hardening pass (dev then prod, probe each). Not RBAC-D scope. **(Confirmed again 19 July 2026, Gate 1 prod probe:** `avatars`, `mosque-photos`, `mosque-madrasa-photos` all have `file_size_limit=null` + `allowed_mime_types=null` — no size/MIME guard. Future hardening item, not today. The new `staff-avatars` bucket (migration 155) DOES carry both, so it's already hardened and excluded from this list.)
- **Gate 2 audit — LOG AND ACCEPT: employment column guard.** *Accepted, no fix now (19 July 2026).* `upsertMosqueStaffEmployment` writes `mosque_staff_employment` via a generic upsert with no server-side column allowlist, but the whole table is owner/admin-only (migration 060 `"Owner manage employment records"` FOR ALL RLS), so the blast radius is contained. Future: wrap it in a field-scoped SECURITY DEFINER RPC (mirroring `update_employee_permissions`) **if** a staff-self write path is ever added. Contrast with `mosque_staff`, which now has the migration-157 privileged-column guard because it is broadly readable/writable.
- **Gate 2 audit — LOG AND ACCEPT: bank details plaintext at rest.** *Roadmap, post-first-customer (19 July 2026).* `mosque_staff_employment.bank_account_name / bank_sort_code / bank_account_number` are stored as plaintext text columns. Owner/admin-only RLS gates access, but the values are not encrypted at rest and bank *changes* are not audited (there is no migration-158 bank-change audit log yet). Move to Supabase Vault / pgsodium column encryption **before** onboarding a paying mosque with real payroll; pair with the deferred migration-158 bank-change audit log (Commit C).
...
### UI overhaul v1 — PRE-MERGE CHECKLIST — ✅ ALL CLEARED, MERGED 18 July 2026
**Squash-merged `ui-overhaul-v1` → `main` as `d540a1b` ("UI overhaul commits 1-5"), pushed → Vercel Production. `rls-154-profiles-can-view` then plain-merged (`ccd21bd`).** Branches `ui-overhaul-v1` + `rls-154-profiles-can-view` FROZEN (not deleted) until the post-merge prod pass is green.
- [x] **Migration 153 (`profiles.dashboard_prefs jsonb`) applied + probed on PROD** (Shiraz-confirmed, raw outputs on record) — dev applied + probed here (column jsonb/nullable/no-default; RLS = owner-self UPDATE `auth.uid()=id` sole write gate, cross-user-write-denied round-trip).
- [x] **Migration 154 (`can_view_profile` profiles RLS PII-leak fix) applied + probed on PROD** (Shiraz-confirmed, raw outputs on record) — dev applied + probed + per-persona round-trip here; zero client code, merged into `main` (`ccd21bd`).
- [x] Shiraz's core browser pass green — staff page ×3 rounds, dashboard empty + populated, compliance numbers vs RPC, fees to the penny.
- [x] `npm run build` green on the merged HEAD (5.64s, pre-push gate).

**⇒ FIRST ITEM NEXT SESSION — deferred UI-overhaul edge-state checks, to be VERIFIED ON PROD** (`youramanah.co.uk`; consciously deferred at merge — accepted risk: no paying users, additive-nullable migrations, single-commit revert via `d540a1b`/`ccd21bd`):
- [ ] **Wrong-level DBS false-positive test** — a DBS at the wrong level must NOT read as a satisfied compliance requirement.
- [ ] **Volunteer RTW exemption** — volunteers are RTW-exempt; confirm they don't surface as an RTW gap.
- [ ] **Limited-role sidebar** — a permission-gated employee sees only allowed destinations (sidebar built from the visible `groups`).
- [ ] **Mosque tab 8-sub overflow** — the Mosque entry's sub-tab bar with 8 subs must not overflow/clip.
Plus Shiraz's own 10-min prod pass: staff numbers on Masjid Nur, card-toggle persistence, mosque2 defaults + teacher-name resolution, deep-URL refresh, six grouped tabs. (Junk-row cleanup SQL runs as its own step AFTER that pass.)

**POST-MERGE PROD PASS — ✅ GREEN (Shiraz, 18 July 2026):** all covered items passed — staff numbers, card-toggle persistence, mosque2 defaults + teacher-name resolution, deep-URL refresh, grouped tabs. Merge is confirmed good on prod. Branches `ui-overhaul-v1` + `rls-154-profiles-can-view` UNFROZEN → **DELETED** (local + origin) now that the pass is green; `main` (`f70d293`) is the single source. Findings surfaced during the pass, carried to NEXT SESSION:
- [ ] **Dev-only footnote renders on the Volunteers page on PROD** — a dev/debug footnote is leaking into the prod Volunteers view. REMOVE next session (find the guard that should have hidden it; likely an unconditional render or a missing `import.meta.env.DEV` gate).
- [x] **Fatima Zahra's `role` field contains a marketplace headline** ("Qualified Quran Teacher for Children | Bradford") — **DIAGNOSED as DATA, not mapping** (staff-profile-v1 / Commit 3): `shapeStaffListRow` does `role: r.role` (straight passthrough) and `mosque_link_scholar_to_staff` (144) sets role from its `p_role` param (default 'Scholar'), NOT from any headline — so a bad value was stored, faithfully displayed. **DISPLAY GUARD SHIPPED** (`cleanRole` in StaffProfile — cut at first `|`, collapse whitespace, 60-char cap; applied in the header + the Employment "Job title" fallback) so the leak can't reach the UI. **Root data cleanup — NOW DOABLE IN-APP:** the new header **Edit** button (identity-edit dialog → `updateMosqueStaff`, no new RPC) lets an owner correct `role` via the constrained Role picker (`ROLE_OPTIONS`). Verified on dev under the owner RLS path: the leaked headline → `role='Teacher'` (+ name/dept/employment_type/start_date) writes cleanly; the `employment_type` CHECK rejects invalid values. Shiraz fixes Fatima Zahra by opening her profile → Edit → pick a role → Save.
- [ ] **Positive volunteer RTW-exemption test still PENDING** — the exemption logic reads right, but there's no real volunteer-type record on prod to prove a volunteer surfaces as RTW-exempt (not a gap). Needs a genuine volunteer record to close (pairs with the deferred "volunteer RTW exemption" edge-state above).

**LAMBDA / VERCEL SLOT FINDING (carried to NEXT SESSION):** the merge deploy still builds **12/12** functions — the old LLM dashboard-briefing endpoint is **still present as an `api/*.js` function** even though Commit 3 removed its only client caller (`getMosqueBriefing` → `admin-brief.js` mode `mosque_ops` is now dead client-side; see the "Freed-MODE" note above). NOTE: that dead branch is a *mode inside `admin-brief.js`*, which stays 12/12 because `admin-brief.js` has 7+ other live callers — so removing the mode frees nothing. The slot-relevant question is whether any WHOLE `api/*.js` file is now call-site-free. **NEXT SESSION:** grep the client for every `/api/<name>` fetch, identify any function file with ZERO remaining call sites, and if one is confirmed dead, **freeze the code + remove the deployment** (delete/rename the `api/*.js`) to drop below 12 and **free a slot — which unblocks wiring the dashboard "Ask a question" (LLM) input** currently disabled for lack of a slot.
