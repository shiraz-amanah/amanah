...
- **Email member invite** — a `community_member_invite` intent in `send-transactional` (no new function). Manual add works today; "invite by email" is the gap.
- **Derived enrolled-parents in the directory** — needs an owner-scoped RPC (**migration 103**) to surface enrolled parents read-only (owner can't freely read parent profiles). Deliberately not silently skipped.
- **Member-count stat tile** on the mosque overview (`MosqueOverview`).
- **Group filter** in the Members directory (needs a member→groups map).
- **Group-targeted announcements + "message all in group"** via existing messaging.
- **Event RSVP** — new `community_event_rsvps` table + RLS + events-component change (member dashboard shows events read-only for now).
- **Passive site-wide geofence** (v1 is Community-tab-scoped); scheduled auto-close cron for sessions (client closes manually / auto-close window today).
