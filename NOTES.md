...
**Follow-up — parent Fees tab (first-class).** Fees buried at the bottom of the child card was poor UX. Added a dedicated **Fees** entry in the parent sidebar (`UserSidebar`, gated on `hasMadrasa`) → new `MadrasaFeesTab.jsx`: ALL children's fees in one place, **Outstanding first** (amber + Pay buttons + total due) then **Paid** history, fed by the 121 RPC and refetching on `paymentSyncTick`. The per-child Fees section in `MadrasaChildProgress` stays (contextual); the tab is the primary place. Browser-verified on dev (sidebar entry, outstanding row + Pay £40, total due, no console errors).
...
54. **⚠️ LEGAL GATE — NO DRAFTS IN-REPO (most urgent).** There is **no `legal/` folder** and **no Privacy Policy / Terms of Service files** anywhere in the repo (verified `git ls-files` + `find`, tracked + untracked). A prior handoff assumed they were committed; in fact they were written in a separate Claude.ai chat as standalone `.docx` downloads and never entered this repo. Both must be **drafted from scratch as version-controlled markdown in a new `legal/`**, scoped to Amanah's real data model (children's data, DBS, safeguarding, photo consent), marked DRAFT. **Blocks the whole legal gate**: ICO registration (**✅ DONE 6 July 2026 — Saveco Tech Ltd, ref C1975988, £52**), solicitor review, and DPAs (Supabase/Vercel/Resend/Daily.co) are not meaningful until the drafts exist. **ICO is now complete; the drafts + DPAs remain** (plus a Privacy Policy voice/audio-data update). Stripe + transcription (2c) unblocked **subject to the DPAs.** Likely a dedicated, properly-scoped future session.