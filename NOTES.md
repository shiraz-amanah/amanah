...
- **Migrate Supabase off the LEGACY `anon`/`service_role` API keys → new publishable/secret keys (Session N1, 16 July 2026).** Supabase disabled the legacy keys on 15 July → every service-role call 401'd platform-wide (surfaced in N1 as a false `404 mosque_not_found` on the WhatsApp send; hit bookings/reminders/staff-messaging/all `sbGet`+`callRpc` intents too). **Currently patched by RE-ENABLING the legacy keys — a stopgap, not a fix.** Supabase can disable them again. Proper fix: swap in the new **secret** key for `SUPABASE_SERVICE_ROLE_KEY` and the new **publishable** key for `SUPABASE_ANON_KEY` / `VITE_SUPABASE_ANON_KEY`, across BOTH Vercel server env AND the client build, on dev then prod, then redeploy + smoke-test a service-role intent. Until done, do NOT let anyone re-disable the legacy keys. Full incident chain in the Session N1 section.
