...
>
> **LOOSE END (safe, deliberate):** `get_mosque_staff_list` **still filters `deleted_at is null`**. Nothing writes that column any more (175 removed the write and cleared existing values), and prod had zero non-null rows, so the filter is **inert** — but it is a dead filter on a superseded concept. Remove it next time that function is opened for another reason; it does not justify a migration of its own. **Next migration: 176.**
>
