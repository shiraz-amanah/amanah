...
- [ ] **Fatima Zahra's `role` field contains a marketplace headline** — a staff/profile `role` value holds leftover marketplace-headline copy, not a role. DATA/MAPPING cleanup (correct the row; check the shaper/mapping that populated `role` from a marketplace field so it can't recur).
