...
- **#1 RTW upload "lookup_failed" — NOT a code bug.** `api/onboarding-upload.js` session lookup (service role) errors only on auth/connectivity, never RLS. Committed only a **diagnostic** improvement (`549af28`): line 86 was the one error branch that swallowed its cause — now logs `sErr.message`+code so the next preview build's logs confirm it. **Does not make the upload work** — that needs the preview key.
