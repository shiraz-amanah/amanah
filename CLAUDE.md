...
- **PROD INCIDENT (15 July 2026) — Supabase disabled the LEGACY `anon`/`service_role` API keys.** Surfaced during Session N1: every service-role call (`sbGet`/`callRpc` in the api functions) 401'd platform-wide, showing up as a false `404 mosque_not_found`. Currently patched by **re-enabling the legacy keys** — a stopgap. **Still owed: migrate to the new publishable/secret keys** across BOTH Vercel server env AND the client build (`VITE_SUPABASE_ANON_KEY`), then redeploy. Until done, do NOT let anyone re-disable the legacy keys.
